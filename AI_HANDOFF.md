...
Current release version: `2026.08.01.001-dawson-moby-pop-up` (prepared locally; not yet live)

Latest process update: August 1, 2026 prepared Dawson's new `Moby Pop-Up` Neighborhood Rotations station. `cafeStationsForWeek()` activates and requires the station only for Dawson weeks starting `2026-08-31`; the UI states that service runs Tuesday through Thursday. Normal `mobyPopUp` state uses a Global-or-`AMZ: Carvery` menu selector and menu-driven 2-entree / 3-side / 2-sub-recipe / 1-extension capacity. The isolated `mobyPopUpPromotionOverride` supports only Tuesday, Wednesday, and Thursday day choices, including one-day promos; when enabled it hides and replaces the entire normal Moby configuration for the saved week. Shared records use dedicated `mobyPopUp` and `mobyPopUpPromotion` station keys and retain the existing Supabase-primary / Smartsheet-mirror rotation contract, so no database schema migration is required. Recall, selected-item metadata, costs/calories/retail, completion, recap, export, and source guards are included. Local verification passed `pnpm run verify`, including the production Vite build, and the Windows-safe equivalents of both required rotation suites passed 31/31 total tests (`neighborhood-rotations.spec.js` plus `reinvent-submit-recall.spec.js`). Independent Reviewer and Verifier both passed after coverage was expanded to select `AMZ: Carvery`, save its normal entrée/side records, clear local cache, and recall the normal menu and selections before exercising promo replacement. Alex authorized deployment. State is `RELEASE AUTHORIZED — PRODUCTION VERIFICATION PENDING`; bind this exact prepared diff to the candidate commit, publish through the Git-backed Vercel production path, then obtain Scribe's independent LIVE sign-off only after production evidence exists.
