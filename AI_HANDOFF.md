...
Current release candidate: `2026.08.23.008-cricket-deli-capacity`

Latest process update: August 23, 2026 prepared Alex's Cricket Deli capacity and completion change as version `2026.08.23.008-cricket-deli-capacity`. North/Cricket now renders six individually labeled Deli LTO slots in every selectable week and requires at least three distinct sandwich selections before Deli completes and the weekly rotation can submit. Repeating a sandwich in multiple slots counts once; the three remaining slots stay optional. The selector authority is isolated to the existing exact `AMZ: Cafe Express Curated Sandwiches` MenuWorks pool (43 verified Deli items); Cricket exposes neither fuzzy fallback rows nor write-ins. The planner explains the Cricket-specific threshold in both the Deli panel and blocked-submit guidance. The shared Deli recall array supports all six deterministic positions so sparse later choices such as slot 6 save and return without compaction. Other cafés keep their current Deli capacities, source behavior, and one-selection completion rule. No schema, source-authority, Supabase/Smartsheet contract, production-data, cost, Results, recap, or export change. `pnpm run verify` passed and both mandatory local browser suites passed 47/47, including six accessible selectors, duplicate selections counting once, three distinct selections satisfying Deli, exact source-pool isolation, sparse slots 1/2/6 persistence, reload recall, and unchanged Moby Deli behavior. Publication and production verification are pending.
