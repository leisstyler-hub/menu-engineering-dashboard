...
Current live version: `2026.08.28.003-ssmt-modifier-row-density`

Latest process update: August 28, 2026 prepared Tyler's SSMT modifier item-line density follow-up as version `2026.08.28.003-ssmt-modifier-row-density`. The modifier detail popup keeps the wider desktop dialog and green row-level `View modifiers` button, while modifier item rows now use tighter cell padding, smaller controls, a shorter resizable description field, an eight-column area-price grid, a compact delete action, and darker slate item/group borders. This is a visual density/readability refinement only: existing modifier group creation, rename/type, copy, delete, item-line editing, exact MRN text, pricing-structure selection, required area price order through MCO, SSMT app-side persistence, Supabase/schema state, production data, Menu Audit source framing, and Webtrition/Menu Library deletion authority are unchanged. Local TDD proof first failed against the previous 185px modifier item row height, then passed after implementation with the item row height at or below 76px and a darker `rgb(203, 213, 225)` divider border. Local SSMT Playwright passed `node scripts/run-playwright.mjs tests/browser/ssmt-tool.spec.js` 6/6. Full `pnpm run verify` remains blocked by pre-existing stale generated-data checks: `build-food-cost-plate-reference.mjs --check` fails first, and `build-menu-cross-utilization-data.mjs --check` also fails when isolated. Release state is `IMPLEMENTED`.
