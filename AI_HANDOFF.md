...
Current release version: `2026.08.04.001-grace-wed-tues-global` (candidate, pending independent review/verification/release)

Latest process update: August 4, 2026 switched East district `Grace`'s Global station cadence to the same single Wednesday-Tuesday cycle as Doppler and Bingo: Monday+Tuesday carry over from the prior Wednesday cycle, and Wednesday starts the next cycle through the following Tuesday. `globalCycleConfig()` and `rotationSummaryBlockLabels()` previously routed only `cafe === "Doppler"` / `cafe === "Bingo"` through the shared Wed-Tues helpers; both now route through one generalized `isWedTuesGlobalCafe(cafe)` check backed by `WED_TUES_GLOBAL_CAFES = new Set(["Doppler", "Bingo", "Grace"])`. Doppler and Bingo's rendered text, save records, and recall behavior are unchanged (the merged branch produces byte-identical output for those two cafes; only the cafe name interpolates). Grace's station list, station slots, and non-Global stations (`streetBeets`, `grill`, `freshFive`, `salad`) are untouched; Grace still uses the standard shared `GlobalSection` UI (not split-global, not Nitro). `scripts/verify-rotation-integrity.mjs` now guards the shared `WED_TUES_GLOBAL_CAFES` set instead of only Bingo's Wednesday cycle. New targeted browser coverage in `tests/browser/neighborhood-rotations.spec.js` proves Grace shows the Wednesday cadence text and Monday + Tuesday carryover panel, and that a saved Grace Global menu/entree selection recalls correctly after reload. No schema, production data, or source-authority change.
