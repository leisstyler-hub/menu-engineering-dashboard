...
Current live version: `2026.08.30.002-ssmt-lock-copy-phase`

Latest process update: August 30, 2026 published Tyler's SSMT width-lock, item lock, copy-to-Centric, phase prominence, price readability, and row-striping follow-up as version `2026.08.30.002-ssmt-lock-copy-phase`. The unlocked SSMT workspace keeps the dense, wide selected-menu editor but caps the desktop surface at `max-w-[2680px]` so a maximized ultra-wide browser no longer stretches the builder across the full viewport. The phase/status control is now a prominent dark panel under the editor navigation, item rows can be locked when Culinary finishes entering them, locked item fields become read-only click-to-copy fields for Centric, area-price cells use slightly larger text and copy their price when clicked on locked rows, locked rows cannot be edited/repriced/deleted until unlocked, and forward phase movement is disabled until every item row is locked. Builder rows also use a clearer alternating row color for scanning. This is an app-side SSMT UI/workflow refinement only: existing menu/modifier editing behavior, exact MRN text, pricing-structure selection, required area price order through MCO, guarded SSMT browser persistence, Supabase/schema state, production data, Menu Audit source framing, modifier behavior, and Webtrition/Menu Library deletion authority are unchanged. Application commit `87ea9e8073f712c1460f83f379b060c06b2f8501` reached GitHub `main`; Vercel production deployment `dpl_Boy5EkSwa57DNV5Qqn7xBTgq3rGb` is `READY`, live HTML references `/assets/index-CUaQZ8c1.js`, that live index contains the version stamp, and the live SSMT chunk `/assets/SsmtTool-LIJ4engG.js` contains the lock/phase/width markers. Local TDD proof first failed against missing lock/copy behavior and the previous 2966.5px builder width at a 3000px viewport, then passed after implementation with phase gating, clipboard copy, larger price text, row striping, locked price/delete freeze, the builder at least table-wide, locked below 2700px, builder item rows at or below 82px, at least 10 visible rows, and no horizontal scroll at that maximized width. Local SSMT Playwright passed `node scripts/run-playwright.mjs tests/browser/ssmt-tool.spec.js` 8/8, and production Playwright passed the same SSMT spec 8/8 against `https://project-d8v25.vercel.app`. Local guards passed: `git diff --check`, `node scripts/build-ssmt-seed-data.mjs --check`, `node scripts/verify-platform-tools.mjs`, `node scripts/verify-menu-audit-tool.mjs`, `node scripts/verify-release-workflow.mjs`, `node scripts/verify-changelog-timestamps.mjs`, and `corepack pnpm run build`. Full `corepack pnpm run verify` remains blocked by the pre-existing stale generated-data guard `build-food-cost-plate-reference.mjs --check`. Release state is `LIVE`.

Previous process update: August 28, 2026 published Tyler's SSMT density follow-up as version `2026.08.28.004-ssmt-maximized-builder-density`. The modifier detail popup keeps the wider desktop dialog and green row-level `View modifiers` button, while modifier item rows now use tighter cell padding, smaller controls, a shorter resizable description field, an eight-column area-price grid, a compact delete action, and darker slate item/group borders. The selected-menu builder now also uses the full maximized desktop width instead of the previous centered max-width surface, trims top-panel spacing, compacts main builder row inputs/actions, switches area prices to a tighter eight-column grid, and darkens builder row borders so more menu lines are visible at once. This is a visual density/readability refinement only: existing menu/modifier editing behavior, exact MRN text, pricing-structure selection, required area price order through MCO, SSMT app-side persistence, Supabase/schema state, production data, Menu Audit source framing, and Webtrition/Menu Library deletion authority are unchanged. Application commit `a2f4adb1db522c90e7a4398dffe51e788be56e42` reached GitHub `main`; Vercel production deployment `dpl_Z94rhLWbhnQ42iVZgQp5iN7eQ7CP` completed successfully, live HTML references `/assets/index-CiXuwr92.js`, and that live asset contains version `2026.08.28.004-ssmt-maximized-builder-density`. Local TDD proof first failed against the previous 185px modifier item row height and the 1943px maximized builder width, then passed after implementation with modifier rows at or below 76px, maximized builder width at least 2800px on a 3000px viewport, builder item rows at or below 82px, at least 10 visible rows, and no horizontal scroll at that maximized width. Local SSMT Playwright passed `node scripts/run-playwright.mjs tests/browser/ssmt-tool.spec.js` 7/7, and production Playwright passed the same SSMT spec 7/7 against `https://project-d8v25.vercel.app`. Local guards passed: `node scripts/build-ssmt-seed-data.mjs --check`, `node scripts/verify-platform-tools.mjs`, `node scripts/verify-menu-audit-tool.mjs`, `node scripts/verify-release-workflow.mjs`, `node scripts/verify-changelog-timestamps.mjs`, and `corepack pnpm run build`. Full `corepack pnpm run verify` remains blocked by the pre-existing stale generated-data guard `build-food-cost-plate-reference.mjs --check`. Release state is `LIVE`.

Previous process update: August 28, 2026 prepared Tyler's SSMT modifier item-line density follow-up as version `2026.08.28.003-ssmt-modifier-row-density`. Application commit `41241b16631d3adc415343e81c18497350525df1` was pushed to GitHub `main`, but Vercel live polling timed out before production verification. This intermediate candidate was immediately superseded by the maximized-builder follow-up above; do not report `.003` as `LIVE` without separate production proof. The scope was limited to modifier popup row density and darker borders, with no Supabase/schema/production-data or source-authority changes.

Previous process update: August 28, 2026 published Tyler's SSMT modifier popup usability follow-up as version `2026.08.28.002-ssmt-modifier-dialog-width`. The row-level `View modifiers` action now uses a green high-contrast treatment so it stands out from secondary row actions, and the modifier detail popup opens in a wider desktop dialog with a tightened fixed modifier table layout so normal desktop use does not require sideways scrolling around the modifier name, description, MRN, calories, price, area prices, and delete action columns. Existing modifier group creation, rename/type, copy, delete, item-line editing, exact MRN text, pricing-structure selection, area price order, SSMT app-side persistence, Supabase/schema state, production data, Menu Audit source framing, and Webtrition/Menu Library deletion authority are unchanged. Application commit `4e27a3e7782ec08f6d8b23f0baf5a0afea51b380` reached GitHub `main`; Vercel production deployment `dpl_5gE6UyxescoeekR5qGAFCXQzczeM` completed successfully and is aliased to `project-d8v25.vercel.app`; the live HTML references `/assets/index-ZzzpQGtq.js`, and the live SSMT chunk `/assets/SsmtTool-BwIsP9jc.js` contains the widened modal and green modifier action classes. Local TDD proof first failed on the existing white modifier button, then passed `node scripts/run-playwright.mjs tests/browser/ssmt-tool.spec.js` 6/6 after implementation. Production Playwright passed the same SSMT spec 6/6 against `https://project-d8v25.vercel.app`. Full `pnpm run verify` remains blocked by pre-existing stale generated-data checks: `build-food-cost-plate-reference.mjs --check` fails first, and `build-menu-cross-utilization-data.mjs --check` also fails when isolated. Release state is `LIVE`.

Previous process update: August 28, 2026 published Tyler's requested Platform Home sectioning and `SSMT` usability/modifier follow-up as version `2026.08.28.001-ssmt-home-sections-modifiers` after Tyler explicitly instructed Chief to finish directly because the normal worker agents were blocked by expired OAuth. Platform Home now groups tools under `Chef Tools` in the order `Neighborhood Rotations`, `Menu Library`, `Menu Engineering`, `Menu Cross Utilization Tool`, `Webtrition`, then `Programming & Auditing` in the order `SSMT`, `Menu Projects`, `Menu Audit Tool`, `Lean Tool`. SSMT now uses a denser four-column menu selector with stronger menu-type contrast, keeps the wide builder table in an internal scroll region, widens/wraps editable item names and descriptions, replaces exact-name deletion with simple click-confirm deletion for app-side SSMT records, and lets users create, rename, type, copy, and delete modifier groups plus add/delete modifier item lines with modifier name, description, exact MRN text, calories, pricing-structure selection, and area prices in the required order `AUS, BNA, BOS, BWI, DEN, IAD, JFK, LAX, SAN, SNA, SEA, SJC, WAS, YVR, YYZ, MCO`. This remains app-side guarded browser persistence, not shared Supabase organizational truth. No Supabase/schema/production-data, Webtrition/Menu Library deletion authority, pricing-source authority, workbook parsing, Menu Audit removal authority, Menu Cross scoring/data logic, or SSMT passcode model changed. Application commit `89ecbc1d03b00e9d270d9d4bf448e9845e29113b` reached GitHub `main`; Vercel production deployment `dpl_GtDEJHDd5D7BmjzzQjbQmXcYgVhf` completed successfully and is aliased to `project-d8v25.vercel.app`; the live public bundle contains the version stamp. Local browser proof passed `node scripts/run-playwright.mjs tests/browser/landing-tool-sections.spec.js` 1/1 and `node scripts/run-playwright.mjs tests/browser/ssmt-tool.spec.js` 5/5. Production Playwright passed the combined focused suites 6/6 against `https://project-d8v25.vercel.app`. Full `pnpm run verify` remains blocked by pre-existing stale generated-data checks: `build-food-cost-plate-reference.mjs --check` fails first, and `build-menu-cross-utilization-data.mjs --check` also fails when isolated. Release state is `LIVE`.

Previous process update: August 27, 2026 published Tyler's requested `SSMT` menu-selector and row-control refinements as version `2026.08.27.002-ssmt-menu-row-ordering`. Menu Selector now groups records in the approved order `Core`, `Global`, `Promotions`, `Thompson Hospitality`, color-codes those groups, and alphabetizes menus within each group. The menu builder now supports adding menu items, deleting item rows, and dragging/dropping item or divider rows to set the IT programming order. The item table now puts blank editable `Fixy` left of `Label`, makes `MRN` a blank editable text field, removes the visible `FOH` label, keeps calories as `N/A` for non-Promotion menus, and places blank editable `Category` and `Secondary category` after SEA price before area prices. Phase/status changes are saved on each app-side menu record and survive SSMT reload through guarded browser persistence. This does not yet make SSMT phase/status a shared multi-user Supabase source of truth; Supabase persistence remains the next backend pass. No Supabase/schema/production-data, pricing-source authority, workbook parsing, Menu Audit removal-authority, or downstream Core/Global eligibility change occurred. Application commit `9025b09affbe38792706b882158f7ac6c95c07aa` reached GitHub `main`; Vercel production deployment `dpl_Cfi71ZvBUjDvoy7vqwaL6EfJf5Lk` completed successfully and is aliased to `project-d8v25.vercel.app`; the live public bundle `/assets/index-Bu84N9fV.js` contains the version stamp. Local SSMT browser proof first failed on missing grouped menu sections, then passed `node scripts/run-playwright.mjs tests/browser/ssmt-tool.spec.js` 3/3. Production Playwright passed the same SSMT spec 3/3 against `https://project-d8v25.vercel.app`. Full `pnpm run verify` remains blocked by pre-existing stale generated-data checks: `build-food-cost-plate-reference.mjs --check` fails first, and `build-menu-cross-utilization-data.mjs --check` also fails when isolated. Release state is `LIVE`.

Previous process update: August 27, 2026 published Tyler's requested `SSMT` flow restructure as version `2026.08.27.001-ssmt-flow-restructure`. After passcode `0411`, SSMT now opens to two primary paths instead of one dense workspace: `Pricing Structure` and `Menu Selector / New Menu`. The pricing view shows the area order `AUS, BNA, BOS, BWI, DEN, IAD, JFK, LAX, SAN, SNA, SEA, SJC, WAS, YVR, YYZ, MCO` and allows app-side pricing rows to be added by SEA price + category. The menu selector opens existing menus or creates a new menu after selecting type. The selected-menu editor is full-width, has a slim navigation bar for back to menu selection, pricing table, and delete menu, and delete requires retyping the menu name exactly. Menu rows now include typed `Category`, a `FOH / Fixy` field, all area prices in the required IT programming order, and addable divider rows with editable titles for Centric category separation inside one menu. Active start/end dates are shown only for `Promotion` and `Thompson Hospitality`; Core and Global show that active dates are not required. This change remains app/workflow UI only: no Supabase schema, production data, source-authority, Menu Audit removal-authority, or seed workbook parsing change. Application commit `68db163d6fedcd51e35b45e668b2e0ed86945cc4` reached GitHub `main`; Vercel production deployment `dpl_3vhmQemNEVmcHhBjxuL86JxbhvLX` completed successfully and is aliased to `project-d8v25.vercel.app`; the live public bundle `/assets/index-CvnbshjJ.js` contains the version stamp and the new SSMT navigation labels. Focused local proof passed `node scripts/run-playwright.mjs tests/browser/ssmt-tool.spec.js` 2/2 after first failing on the missing two-button split, and production Playwright passed the same SSMT spec 2/2 against `https://project-d8v25.vercel.app`. Full `pnpm run verify` remains blocked by the pre-existing stale generated food-cost reference guard before it reaches SSMT. Release state is `LIVE`.

Previous process update: August 27, 2026 published Tyler's approved `SSMT` replacement tool and Menu Audit source-model reset as version `2026.08.26.001-ssmt-tool-menu-audit-source`. Platform Home now includes a passcode-gated `SSMT` card; the tool opens with shared passcode `0411`, loads the workbook-derived seed payload from `public/data/ssmtSeedData.json`, displays the required pricing area order with MCO last, assigns item prices only through pricing-structure SEA price + category selectors, keeps unmatched workbook price values as review context instead of assigned prices, keeps active start/end dates editable on each menu record, normalizes labels to ALL CAPS, normalizes descriptions to lower case, keeps calories promotional-only, opens modifier details in a modal, records copied modifier groups as independent-copy behavior, and creates saved flag/report email drafts for Alex and Tyler. `scripts/build-ssmt-seed-data.mjs` parses `docs/ssmt/SEA Standard Menu Template (1).xlsx`, reads Navigation UI drawing hyperlink targets, includes approved visible recent/promotion sheets, and currently produces 56 menu records, 592 modifier groups, 62 pricing rows, and 61 Navigation UI button targets; `pnpm run verify` checks that generated data is current. Menu Audit now presents the intended operating model as SSMT app records + Webtrition Report Menu Index + Shopping Lists, with recipes called out as the remaining missing data layer. Tyler narrowed deletion authority in-thread: the uploaded SSMT workbook is initial tool/price/modifier/candidate alignment input, not item-removal authority. Webtrition Report Menu Index remains the operational removal authority, and SSMT/Webtrition differences are review flags until confirmed. Application commit `79a0656b0e1f38fdc691f6731266e6860d11a125` reached GitHub `main`; Vercel production deployment `dpl_82oFq2v5Ga5AbTUn3qvXkwGZ3K9Z` completed successfully, the public bundle at `https://project-d8v25.vercel.app/assets/index-D_Fx3shT.js` contains the version stamp, live seed data contains 56 menu records, 592 modifier groups, 62 pricing rows, and 61 Navigation UI targets with zero unmatched raw workbook prices assigned, and production Playwright passed `tests/browser/ssmt-tool.spec.js` 2/2. Normal Reviewer, Verifier, and Release sessions were unavailable with expired OAuth; Tyler explicitly instructed Chief to continue directly. Release state is `LIVE`.

Previous process update: August 23, 2026 published Alex's Cricket Deli capacity and completion change as version `2026.08.23.008-cricket-deli-capacity`. North/Cricket now renders six individually labeled Deli LTO slots in every selectable week and requires at least three distinct sandwich selections before Deli completes and the weekly rotation can submit. Repeating a sandwich in multiple slots counts once; the three remaining slots stay optional. The selector authority is isolated to the existing exact `AMZ: Cafe Express Curated Sandwiches` MenuWorks pool (43 verified Deli items); Cricket exposes neither fuzzy fallback rows nor write-ins. The planner explains the Cricket-specific threshold in both the Deli panel and blocked-submit guidance. The shared Deli recall array supports all six deterministic positions so sparse later choices such as slot 6 save and return without compaction. Other cafés keep their current Deli capacities, source behavior, and one-selection completion rule. No schema, source-authority, Supabase/Smartsheet contract, production-data, cost, Results, recap, or export change. Application commit `745bd2e10ae28e5be570d35427b6a50688242e15` reached GitHub `main`; Vercel deployment `CRoWGbCH3EsubSB6xE2Vkgpb4Hc8` completed successfully, and the public bundle at `https://project-d8v25.vercel.app` contains the release version. `pnpm run verify` passed and both mandatory browser suites passed 47/47 locally and 47/47 in production, including six accessible selectors, duplicate selections counting once, three distinct selections satisfying Deli, exact source-pool isolation, sparse slots 1/2/6 persistence, reload recall, and unchanged Moby Deli behavior. Release state is `LIVE`.

Previous process update: August 23, 2026 publishes Tyler's Pair Detail display refinement for Menu Cross Utilization as version `2026.08.23.007-menu-cross-pair-detail-scroll`. The change is limited to `src/features/menu-cross-utilization/MenuCrossUtilizationTool.jsx`, `tests/browser/menu-cross-utilization.spec.js`, the app version stamp, and release records. The pair-detail panel now uses a taller shared-ingredients scroll region for long menu pairs and keeps the blue `Ordering / reuse opportunity` note at the bottom of the detail panel. The matrix grouping, pillar coloring, same-menu `100%` diagonal behavior, shopping-list source data, score formula, generated JSON, Supabase/schema/production-data state, home tile, and mobile bottom nav are unchanged. Local TDD proof first failed against the current detail panel, then passed `node scripts/run-playwright.mjs tests/browser/menu-cross-utilization.spec.js -g "long pair details"` after implementation.

Previous process update: August 23, 2026 publishes Tyler's second Menu Cross Utilization display refinement as version `2026.08.23.006-menu-cross-pillar-grouped-matrix`. The change is limited to `src/features/menu-cross-utilization/MenuCrossUtilizationTool.jsx`, `tests/browser/menu-cross-utilization.spec.js`, the app version stamp, and release records. The Pillar Strategy cards now explicitly label `Pillar cross-utilization %` and include each menu's in-pillar cross-use percent in the compact menu chips. The full menu list now includes a `Pillar Cross-Use %` column separate from portfolio-wide cross-use. The Pairwise Matrix now sorts menus by document-defined pillar, adds a color-coded pillar header band and row/column color cues, uses wider 44px cells on a wider page surface, and renders same-menu diagonal cells as non-clickable `100%` matches with no pair-detail popup. The shopping-list source data, score formula, generated JSON, shared-ingredient pair details, `Chickle` no-data behavior, no-prep/no-recipe/no-labor/no-cost/no-savings/no-waste guardrails, Supabase/schema/production-data state, home tile, and mobile bottom nav are unchanged. Local TDD proof first failed on the missing overview `Pillar cross-utilization %` marker, then passed `node scripts/run-playwright.mjs tests/browser/menu-cross-utilization.spec.js` 4/4 after implementation.

Previous process update: August 23, 2026 published Alex's North-only duplicate Global-menu policy as version `2026.08.23.005-north-duplicate-menu-policy`. A centralized `DUPLICATE_MENU_ALLOWED_DISTRICTS` policy now excludes North rows from cross-café conflict counts and candidate submit blockers, so two or more North cafés may use the same Global Menu in the same week without a duplicate warning. The existing South controlled-café rule, Re:Invent exception, East `AMZ: Balti` exception and non-Balti enforcement, and uniqueness between split Global blocks within one café remain unchanged. No selection, persistence, Supabase/Smartsheet authority, schema, production data, Results, recap, cost, or export contract changes. Application commit `a5f9ce77135c4844d54da51446feca39424aa4d2` reached GitHub `main`; Vercel deployment `3a4a9CYaT9JyHwMMKq5cuThvDHZL` completed successfully, and the public bundle at `https://project-d8v25.vercel.app` contains the release version. `pnpm run verify` passed, and both mandatory browser suites passed 46/46 locally and 46/46 in production, including the North allowed-duplicate case, East non-Balti enforcement, split-block uniqueness, soup planner, reference plate costs, Moby projection, and recall regressions. Release state is `LIVE`.

Previous process update: August 23, 2026 published Alex's planner-wide weekly soup schedule as version `2026.08.23.004-weekly-soup-planner`. Every café and selectable week now receives one shared optional Soup LTO station with ten slots grouped as two soups for each Monday-through-Friday service day. The selector pool is isolated to exact `AMZ: Cafe Express Soup` rows; write-ins are disabled, non-soup menu items cannot appear, and legacy Fresh Five/fuzzy soup rows are omitted from display, costing, and the next replacement save. Each selector has a weekday-specific accessible name. Soup selections use the source 12 oz true cost and approved $5.00 retail, while the planner derives 16 oz true cost proportionally and displays it against the approved $6.10 retail, with food-cost percentages for both sizes; a missing source cost remains unavailable/blank rather than becoming 0%. Selected soups retain descriptions, calories, diet/allergen details, and save/recall as existing station-selection records with exact source menu/station/MRN/portion plus deterministic slot numbers, including sparse Friday slot 10; no schema migration or new table is required. Supabase remains primary and Smartsheet remains fallback/mirror. Soup is excluded from submission requirements and required-station progress for every café, including when all ten slots are empty. Application commit `adc1c3515db8e1c5e5b90fad4c9a59fbaddaf9f4` reached GitHub `main`; Vercel deployment `EDMxq37AVGC3P4qmyDxEcKhPiRMX` completed successfully, and the public bundle at `https://project-d8v25.vercel.app` contains the release version. `pnpm run verify` passed, and both mandatory browser suites passed 45/45 locally and 45/45 in production, proving the five two-slot day groups, exact menu isolation/legacy cleanup, portion math, blank missing-cost persistence, source identity, sparse save/recall, optional submission/progress, accessibility labels, presence across every café, and existing recall flows. Release state is `LIVE`.

Previous process update: August 23, 2026 publishes Tyler's home Changelog display correction as version `2026.08.23.003-landing-changelog-compact`. The landing page now parses detailed `CHANGELOG.md` release records into compact dashboard summaries, caps rendered summary lines, limits the desktop Changelog panel height with internal scrolling, and prevents the changelog grid row from stretching the panel to match neighboring dashboard content. The durable `CHANGELOG.md` audit entries remain detailed; only the home dashboard presentation is shortened. Scope is limited to `src/app/LandingPage.jsx`, `tests/browser/landing-changelog.spec.js`, the app version stamp, and release records. Menu Cross Utilization data, scoring, matrix behavior, source PDFs, generated JSON, Supabase, schemas, production data, navigation, and mobile bottom nav are unchanged. Local TDD proof first failed because the home page exposed the long audit phrase `The score formula...`, then passed `node scripts/run-playwright.mjs tests/browser/landing-changelog.spec.js` after implementation.

Previous process update: August 23, 2026 published Tyler's Menu Cross Utilization matrix-display refinement as version `2026.08.23.002-menu-cross-utilization-matrix-display`. The change is limited to `src/features/menu-cross-utilization/MenuCrossUtilizationTool.jsx`, `tests/browser/menu-cross-utilization.spec.js`, the app version stamp, and release records. The Pairwise Matrix now uses a red heatmap instead of teal/blue, uses larger 38px cells, prints the overlap percent directly inside each square, and shows a matrix-view `Pillar cross-utilization %` summary for each document-defined pillar. The score remains the same plain Jaccard ingredient/purchasing overlap on match keys; the source data, PDF parser, Chickle no-data handling, no-prep/no-recipe/no-labor/no-cost/no-savings/no-waste guardrails, Supabase/schema/production-data state, home tile, and mobile bottom nav are unchanged. Tyler explicitly instructed Chief to do the follow-up directly while worker agents were session-limited. Local TDD proof first failed on the missing `Pillar cross-utilization %` matrix summary, then passed `node scripts/run-playwright.mjs tests/browser/menu-cross-utilization.spec.js` 3/3 after implementation.

Previous process update: August 23, 2026 published the `Menu Cross Utilization Tool`, an 8th Platform Home tile, for Tyler's menu-planning cross-utilization mission (approved design thread rooted at event `40a0250551978a1a19ad9314b29a971fe7e0feabe3028225cf339cb399836577`). Phase 1 scope is read-only and build-time generated: `scripts/build-menu-cross-utilization-data.mjs` parses the 55 checked-in `docs/menu-cross-utilization/shopping-lists/*.pdf` MenuWorks Shopping List Reports for `Amazon Region (FBE000)`, `Aug 3, 2026`, `Lunch` using `pdfjs-dist` positioned-text extraction (category headers and description-wrap continuation lines are reconstructed by font suffix `_f1`/`_f3` and column x-position, not naive `pdftotext -layout`), and writes the committed, `--check`-verifiable `src/data/menuCrossUtilization.json`. Match key is `MIT-<code>` when present, else a documented case/punctuation-insensitive normalized-description fallback. Low-signal pantry items (water, salt, generic granulated sugar, generic black pepper, broad cooking oils, cooking spray) are excluded from the primary score per the workbook's own methodology sheet; distinctive variants (sesame oil, brown sugar, specialty spices, cuisine vinegars) remain eligible. The only score is plain Jaccard overlap on match-key sets (shared / union) — no prepared-component weighting, no prep/recipe/labor/cost/savings/waste claims anywhere in the UI copy. `Chickle` (0 extracted ingredient rows, confirmed both by its own PDF's "no data" page and the workbook's Source Audit sheet) stays visible in the menu list with an explicit "no ingredient data" state and is excluded from the pairwise matrix grid rather than shown as a misleading all-zero row. The tool has two screens: Pillar Strategy (8 document-defined cuisine pillars pulled as static reference data from the workbook's `01 Pillar Summary` sheet, each showing this tool's own computed avg intra-pillar overlap) + the full 55-menu list, and a 54x54 (Chickle excluded) hover/click Pairwise Matrix whose pair-detail panel shows, in order: % overlap, both menus' eligible-SKU counts (so small menus aren't over-read), the shared-ingredient list, and a purchasing/ordering-reuse note. `node scripts/build-menu-cross-utilization-data.mjs --check` and `node scripts/verify-menu-cross-utilization.mjs` are wired into the `verify` npm script chain; the verify script sample-checks generated eligible-SKU and shared-SKU counts for Andes/Anisa/Ciudad/Carvery against the workbook (an independent LLM-derived one-time extraction used only as a validation reference, not primary truth — generated counts land within a documented ±3 tolerance across all 8 sampled comparisons, exact on 3 of 8 and off by 1-2 on the rest). New Playwright coverage in `tests/browser/menu-cross-utilization.spec.js` covers tool open, pillar view, the full menu list including Chickle's no-data state, matrix render/Chickle exclusion, pair-detail field order, and tablet/mobile viewports. `pdfjs-dist` was added as a devDependency for this build-time parser only (Tyler's explicit approval in-thread) and is not part of the shipped client bundle's import graph outside the lazy-loaded tool chunk's bundled JSON. Existing 7 tools, their layout, the 5-item mobile bottom nav, and the Supabase-primary/localStorage-never-source-of-truth rule are unchanged; `PRODUCT_DECISIONS.md`, `CHANGELOG.md`, and the application version stamp are reconciled. Normal Reviewer, Verifier, fallback reviewer, and Release agents did not respond in-thread; Tyler explicitly authorized Chief to act as emergency release executor. Chief verified the PR head, reproduced the unrelated stale food-cost reference gate on the PR base, pushed application commit `aa16347f74dbf3b898e24672f8fb7468b12d9f62` to GitHub `main`, confirmed PR #5 closed/merged, confirmed the live public bundle contained version `2026.08.23.001-menu-cross-utilization-tool`, and production Playwright coverage for `tests/browser/menu-cross-utilization.spec.js` passed 3/3 against `https://project-d8v25.vercel.app`. Release state is `LIVE`.

Previous process update: August 16, 2026 published Alex's four reference plate-cost corrections. The Markdown authority and generated 1,566-row reference now use `Sub Recipe` everywhere the source previously said `Plate Add`, and the generator rejects a reintroduced legacy label. House of Teriyaki therefore satisfies its one composed-component requirement from the planner's Sub Recipes field; Anisa satisfies its two Sub Recipes and two Sides across the combined Lebanese/Persian planner while retaining same-menu isolation. Dedicated Fish Market LTO panels require no new operator fields: they automatically enumerate two distinct Fish Market sides plus one sauce, exclude Lemon Wedge from the sauce pool, display named lowest/highest plate endpoints, and persist only the actual LTO choice. Core Grill requires no side selection and displays four named plate prices per sandwich from the two lowest-cost and two highest-cost unique Grill Core sides; duplicate Garden Salad reference rows count once, and inferred sides are never persisted. Promotions, extensions, completion, Results, recaps, summaries, exports, schemas, and Supabase/Smartsheet authority remain unchanged. Application commit `65ea8568b9e67ebce73e10f2e717e2e6f1ee774c` reached GitHub `main` and Vercel production deployment `5933244767` successfully at `https://project-d8v25-j129kyyzo-tylerl-s-projects.vercel.app`; the public bundle contains `2026.08.16.002-reference-plate-component-corrections`. Both required production browser suites passed 43/43 against `https://project-d8v25.vercel.app`, including the exact Re:Invent Fish Market/Core Grill and Nitro Teriyaki/Blueshift Anisa corrections, normalized Sub Recipe save/recall, inferred-component non-persistence, promotion/extension boundaries, and clean page/runtime collectors. Release state is `LIVE`.

Previous process update: August 16, 2026 published Alex's approved expansion of the reference-driven plate-cost interface to every current or future Neighborhood Rotations planner whose selected or fixed menu exists in the generated food-cost reference. `PlannerReferencePlateCost` is menu-gated and resolves existing station selections back to exact reference identities by menu, station, MRN, portion, then item name. It supports standard, Nitro, split, and secondary Global planners plus Wok, Carvery, Moby Pop-Up, menu-backed LTOs, Lotus W&P, and explicitly named reference Commissary menus across every district/café. Valid plates use each primary row's Plate Build, Item + Waste Cost, every valid selected required-component combination, and the primary Sell Price denominator. Extensions remain excluded and display separate economics. Incomplete identity/cost/sell data, including a resolved entrée/plate that lacks a sell price or usable individual Plate Build, renders `Reference data unavailable` and suppresses every summary and per-primary calculated range, including mixed selections where another plate is otherwise complete. Promotion overrides suppress the interface; non-reference menus and historical weeks retain legacy analytics. The expansion remains planner-only: selectors and persisted records are unchanged outside the already-published exact Nessie Aug 17 picker pilot, while recaps, Results, summaries, exports, schemas, Supabase authority, and Smartsheet fallback remain unchanged. Application commit `2fa7c18804bf6347d32f3cf119ef63335a19cea9` reached GitHub `main` and Vercel production deployment `5932762278` successfully; the public bundle contains version `2026.08.16.001-reference-plate-cost-rollout`. Both required browser suites passed 41/41 against `https://project-d8v25.vercel.app` (Neighborhood Rotations 19/19; Re:Invent recall 22/22), covering reference math and extension economics, mixed and resolved-non-calculable suppression, promotions, historical and non-reference legacy behavior, all named planner paths, persistence/Results isolation, and clean page/runtime error collectors. Documentation closeout commit `d9c882a973b1d0fe53a6a7803d0a6f9a2269408e` was published successfully. Release state is `LIVE`.

Previous process update: August 15, 2026 published the revert of the `.003` compact-card experiment in the exact North/Nessie/Global `Aug 17, 2026 - Aug 21, 2026` food-cost pilot at Alex's request. `ReferencePlateCostAnalytics` is restored to the exact pre-compact `.002` presentation: the larger outer inset, header, badge, summary tiles, per-primary cards, value typography, gaps, and extension rows are back. The temporary opt-in `compact` branch was removed from shared `Mini`, so its original implementation is restored as well. Plate math, reference rows, selectors, menu/station/MRN/portion isolation, header ranges, extension treatment, save/recall, Supabase/Smartsheet authority, and all other cafés/weeks remain unchanged. The browser regression verifies the restored larger computed insets/radius and horizontal containment, and the integrity guard protects the spacious pre-compact classes. Application commit `04b6c1e085e11a9f000173d66f702363b453afb3` reached Vercel production deployment `5922453651` successfully, the live bundle contains version `2026.08.15.004-nessie-plate-cost-card-revert`, and both required browser suites passed 39/39 against `https://project-d8v25.vercel.app`, including restored geometry/containment and unchanged reference math, save/recall, extension separation, and adjacent-week isolation. Release state is `LIVE`.

Previous process update: August 15, 2026 published compact light-blue analytics cards inside the exact North/Nessie/Global `Aug 17, 2026 - Aug 21, 2026` food-cost pilot. The panel retains its white/sky palette and every existing label/value, but uses a tighter outer inset, smaller header and badge, two compact summary tiles, denser per-primary result cards, and slimmer extension rows. The shared `Mini` component gained an opt-in `compact` presentation used only by these two pilot summary tiles, so all other analytics surfaces remain unchanged. Plate costs, percentages, reference identities, save/recall behavior, header economics, source authority, scope, and extension exclusion are unchanged. The browser regression checks the compact one-rem panel / three-quarter-rem card spacing and radius against maximum computed dimensions plus horizontal content containment; the integrity guard protects the pilot-only compact styling. Application commit `abedf8d1d2150c5599685b458a410eda18e78cf2` reached Vercel production deployment `5922359260` successfully, the live bundle contains version `2026.08.15.003-nessie-plate-cost-compact-cards`, and both required browser suites passed 39/39 against `https://project-d8v25.vercel.app`, including exact compact geometry/containment plus the unchanged reference math, persistence, recall, and adjacent-week behavior. Release state is `LIVE`.

Previous process update: August 15, 2026 published a reference-driven replacement for the exact North/Nessie/Global week beginning `2026-08-17`. `scripts/build-food-cost-plate-reference.mjs` converts all 1,566 full-index Markdown rows plus the 53-row Concept Guide into `src/data/foodCostPlateReference.json`; `pnpm run verify` checks that generated data remains byte-for-byte current with the Markdown authority. The pilot filters out every intentionally unclassified `AMZ+RA:` concept and offers only concepts with a priced `Entree` or `Plate` primary and a usable build, because Alex requires that row's `Sell Price` as the denominator. A row-level `Plate Build` is authoritative; when it is blank, only a simple Concept Guide build that is neither `Unknown` nor `Varies by station` may fill it. Multi-station concepts expose an explicit Reference Station selector; a required component absent from that sub-station may fall back only within the same menu. Choices remain isolated by menu + station + exact MRN + portion. Selectors come from each selected primary row's effective build, and the calculator finds the minimum/maximum `Item + Waste Cost` across every valid selected-component combination. Extensions are never plate components and instead render separate individual cost/sell/food-cost economics. Pilot selection records continue through the existing flexible rotation record contract, save exact station/MRN/portion data (including cross-station fallback components), and recall the primary Reference Station plus every isolated reference identity independently of backend row order without a schema migration. Saved rotation-header cost ranges use the same reference calculator and stay blank unless every selected primary has a complete calculation. The direct browser regression covers Smokehouse's two entrées + two-of-three Sides + Cornbread + one-of-two Plate Adds, exact save/reload identity and header analytics, incomplete multi-primary header suppression, separate extension economics, Ohana/Lotus Jasmine Rice isolation, Wok station choices, Concept Guide-backed Breakfast Plate math with component-first shuffled recall, RA exclusion, and adjacent-week legacy analytics. Application commit `d46fe818de042d68b74ea2b2e3f38816ea999e0b` reached Vercel production successfully at `https://project-d8v25-7okm441ky-tylerl-s-projects.vercel.app` (GitHub deployment `5922190699`); the live bundle contains version `2026.08.15.002-nessie-reference-plate-builds` and the reference pilot markers. Both required browser suites passed 39/39 against `https://project-d8v25.vercel.app`, including the exact pilot, deterministic save/recall, header completeness, extension separation, RA exclusion, and adjacent-week isolation. Release state is `LIVE`.

Previous process update: August 15, 2026 published `docs/FOOD_COST_PLATE_COSTING_REFERENCE.md`, a durable Markdown conversion of Tyler's reviewed `WebT Price Average AI Reference (1).xlsx` workbook for future agent food-cost calculation work. The reference preserves all 1,566 source item rows across 53 concepts, treats MRNs as exact text, defines `Item + Waste Cost` as the plate-cost input, records confirmed non-RA plate formats (including Soup, Pizzas & Flatbreads, Taco Total, Tavola Nova, Yakisoba, and station-grouped Street Eats/Wok), and explicitly forbids inference for all `AMZ+RA:` concepts. Taco Total's canonical component spelling is `Protein`. Compatibility/grouping rules, special rules, and source-cost-gap remediation remain pending beyond the confirmed direction. This is a documentation-only release: application code, Supabase, Smartsheet, and menu source records are unchanged. Markdown validation confirmed 1,566 full-index rows, 53 concepts, and 14 intentionally unclassified RA concepts; the production build passed. GitHub `main` application/docs commit `5fce98490851338ea51b7152fff39f8dc47439db` is deployed by Vercel production deployment `dpl_6hgjXVt3tVX9Mp5ufXZPasQVvToY` (`READY`), and live asset `/assets/index-CMt200rJ.js` contains version `2026.08.15.001-food-cost-plate-reference`. Release state is `LIVE`.
...
- Build SSMT menu/programming records for Culinary, Experience, and IT.
- Audit SSMT app records against Webtrition metadata, Shopping Lists, and Centric brand report alignment.
...
File area: `src/features/ssmt/`, `docs/ssmt/`, `public/data/ssmtSeedData.json`, `scripts/build-ssmt-seed-data.mjs`

Purpose: Replace the working SSMT Excel workflow with an in-app Culinary-to-IT programming tool.

Current implementation state:

- The tool is available from Platform Home as `SSMT` and uses shared passcode `0411` as a convenience gate, not user identity.
- Seed data is parsed from `docs/ssmt/SEA Standard Menu Template (1).xlsx` into `public/data/ssmtSeedData.json`; the current parse reads Navigation UI drawing hyperlink targets plus approved visible recent/promotion sheets and yields 56 menus, 592 modifier groups, 62 pricing rows, and 61 Navigation UI button targets.
- The uploaded workbook is seed/structure/pricing/modifier/candidate-review input only. It is not deletion authority for operational Menu Library/Webtrition records.
- Webtrition Report Menu Index remains removal authority until Tyler explicitly changes that rule.
- Price assignment uses pricing-structure SEA price + category selectors, then shows all area columns in order: `AUS, BNA, BOS, BWI, DEN, IAD, JFK, LAX, SAN, SNA, SEA, SJC, WAS, YVR, YYZ, MCO`. Workbook item prices that do not match the pricing structure are review context only and are not assigned prices.
- Active start and active end dates appear only for Promotion and Thompson Hospitality menu records; Core and Global menus show that active dates are not required.
- Menu labels normalize to ALL CAPS; descriptions normalize to lower case; calories are promotional-menu-only.
- Modifier detail opens in a modal; copied modifier groups must be independent copies.
- Menu rows include typed Category, FOH / Fixy, and all area prices in the required IT programming order. Divider rows can be added and titled to separate Centric categories inside one menu.
- Flagging uses saved in-app state plus a report email draft to `alexander.neuse@compass-usa.com` and `tyler.leiss@compass-usa.com`; email is notification, not source of truth.
- Core/Global records become downstream-eligible only after `IT complete`; Thompson Hospitality and Promotion records remain saved historical SSMT records but are excluded from operating selectors.

Important tests:

- `tests/browser/ssmt-tool.spec.js`
- `scripts/build-ssmt-seed-data.mjs --check`

### Menu Audit Tool

File area: `src/features/menu-audit/`

Purpose: Compare SSMT app records against Webtrition Report Menu Index metadata, Webtrition Shopping Lists, and uploaded Centric Brand Reports.

Critical integrity rules:

- MRNs are text. Never round or shorten MRNs.
- SSMT data should include modifier item names and mark modifiers as modifiers.
- SSMT-only and Webtrition-only differences are review flags unless the Webtrition Report Menu Index confirms removal authority.
- Recipes are the remaining missing data layer.
...
- Menu Audit Tool has the new SSMT/Webtrition source framing, but durable SSMT persistence and full Webtrition-record reconciliation still need backend follow-through before the tool can replace every workbook/import operating path.
