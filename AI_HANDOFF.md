...
Current prepared version: `2026.08.16.001-reference-plate-cost-rollout` (implemented, locally verified, and approved by Alex for publication; awaiting independent release clearance and deployment)

Latest process update: August 16, 2026 prepared Alex's approved expansion of the reference-driven plate-cost interface to every current or future Neighborhood Rotations planner whose selected or fixed menu exists in the generated food-cost reference. `PlannerReferencePlateCost` is menu-gated and resolves existing station selections back to exact reference identities by menu, station, MRN, portion, then item name. It now supports standard, Nitro, split, and secondary Global planners plus Wok, Carvery, Moby Pop-Up, menu-backed LTOs, Lotus W&P, and explicitly named reference Commissary menus across every district/café. Valid plates continue to use each primary row's Plate Build, Item + Waste Cost, every valid selected required-component combination, and the primary Sell Price denominator. Extensions remain excluded and display separate economics. Incomplete identity/cost/sell data, including a resolved entrée/plate that lacks a sell price or usable individual Plate Build, renders `Reference data unavailable` and suppresses every summary and per-primary calculated range, including mixed selections where another plate is otherwise complete. Promotion overrides suppress the interface; non-reference menus and historical weeks retain legacy analytics. This expansion is planner-only: selectors and persisted records remain unchanged outside the already-published exact Nessie Aug 17 picker pilot, while recaps, Results, summaries, exports, schemas, Supabase authority, and Smartsheet fallback remain unchanged. Direct browser coverage proves Dawson reference math and extension economics, mixed-selection range suppression, resolved-but-non-calculable Breakfast handling, non-pilot header/Results isolation, reference-unavailable messaging, promotion suppression, historical isolation, and standard/split/Nitro/Moby/LAX/station-family reach. `pnpm run verify` passed, including the 1,566-row reference freshness gate and production build, and both required local browser suites passed 41/41 (Neighborhood Rotations 19/19; Re:Invent recall 22/22). Version `2026.08.16.001-reference-plate-cost-rollout` is implemented and locally verified. Alex has now explicitly authorized publication; the exact candidate is awaiting independent release clearance and remains not yet committed, pushed, deployed, or live.
