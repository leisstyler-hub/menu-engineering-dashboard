...
Current release candidate: `2026.08.23.005-north-duplicate-menu-policy`

Latest process update: August 23, 2026 prepared Alex's North-only duplicate Global-menu policy as version `2026.08.23.005-north-duplicate-menu-policy`. A centralized `DUPLICATE_MENU_ALLOWED_DISTRICTS` policy now excludes North rows from cross-café conflict counts and candidate submit blockers, so two or more North cafés may use the same Global Menu in the same week without a duplicate warning. The existing South controlled-café rule, Re:Invent exception, East `AMZ: Balti` exception and non-Balti enforcement, and uniqueness between split Global blocks within one café remain unchanged. No selection, persistence, Supabase/Smartsheet authority, schema, production data, Results, recap, cost, or export contract changes. Direct browser coverage proves a Nessie candidate can match a submitted Dawson Global menu without receiving a duplicate blocker while its unrelated missing-station requirements remain active; the existing East non-Balti regression continues to prove that other-district enforcement is preserved. `pnpm run verify` passed, and both mandatory local browser suites passed 46/46. Publication and production verification are pending.
