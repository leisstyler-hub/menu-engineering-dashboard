...
function LandingPage({ onOpenMenuEngineering, onOpenNeighborhoodRotations }) {
  return (
...
const ROTATION_WEEKS = [
  "P9 WK2 '26", "P9 WK3 '26", "P9 WK4 '26",
  "P10 WK1 '26", "P10 WK2 '26", "P10 WK3 '26", "P10 WK4 '26",
  "P11 WK1 '26", "P11 WK2 '26", "P11 WK3 '26", "P11 WK4 '26",
  "P12 WK1 '26", "P12 WK2 '26", "P12 WK3 '26", "P12 WK4 '26",
  "P13 WK1 '26", "P13 WK2 '26", "P13 WK3 '26", "P13 WK4 '26",
  ...Array.from({ length: 52 }, (_, index) => `P${Math.floor(index / 4) + 1} WK${(index % 4) + 1} '27`)
];
...
function NeighborhoodRotations({ onBackToPlatform }) {
  ...
  const [week, setWeek] = useState(ROTATION_WEEKS[0]);
  const [selectedCafe, setSelectedCafe] = useState("Doppler");
  ...
  const [rotations, setRotations] = useState(() => {
    try { return JSON.parse(localStorage.getItem("neighborhoodRotations_v5") || "{}"); } catch { return {}; }
  });

  useEffect(() => { localStorage.setItem("neighborhoodRotations_v5", JSON.stringify(rotations)); }, [rotations]);

  const cafes = DISTRICTS[district] || [];
  useEffect(() => { if (!cafes.includes(selectedCafe)) setSelectedCafe(cafes[0] || ""); }, [cafes, selectedCafe]);

  const currentRotation = rotations[rotationKey(week, district, selectedCafe)] || EMPTY_ROTATION;
        ...
        {rotationView === "planner" && (
          <>
            <section className="grid grid-cols-1 lg:grid-cols-4 gap-4">
              <ControlCard label="District" value={district} setValue={setDistrict} options={Object.keys(DISTRICTS)} />
              <ControlCard label="Week" value={week} setValue={setWeek} options={ROTATION_WEEKS} />
              <ControlCard label="Café" value={selectedCafe} setValue={setSelectedCafe} options={cafes} />
              <StatusCard conflicts={Object.values(conflictMenus).filter((count) => count > 1).length} completed={districtWeekRows.filter((row) => row.menu).length} total={cafes.length} />
            </section>
            <section className="grid grid-cols-1 xl:grid-cols-3 gap-6">
              <div className="xl:col-span-2 space-y-6">
                <RotationPlannerCard cafe={selectedCafe} menuOptions={menus} rotation={currentRotation} updateRotation={updateRotation} week={week} printRows={allRows} />
              </div>
...
function ControlCard({ label, value, setValue, options }) {
  return (
    <div className="rounded-3xl bg-white border border-slate-200 p-5 shadow-xl">
...
function StatusCard({ conflicts, completed, total }) {
  return (
    <div className="rounded-3xl bg-white border border-slate-200 p-5 shadow-xl">
...
function RotationPlannerCard({ cafe, menuOptions, rotation, updateRotation, week, printRows }) {
  const [preview, setPreview] = useState(null);
  const [copiedRotation, setCopiedRotation] = useState(() => {
    try { return JSON.parse(localStorage.getItem("copiedNeighborhoodRotation") || "null"); } catch { return null; }
  });
  ...
  const markDraft = () => updateRotation({ status: "Draft", updatedAt: nowStamp(), submittedBy: "Chef" });
  const submitRotation = () => {
    if (!canSubmitRotation) return;
    const stamp = nowStamp();
    updateRotation({ status: "Submitted", updatedAt: stamp, submittedAt: stamp, submittedBy: "Chef" });
  };
  const copyCurrentRotation = () => {
    const copy = { ...rotation, status: "Draft", updatedAt: "", submittedAt: "", submittedBy: "" };
    localStorage.setItem("copiedNeighborhoodRotation", JSON.stringify(copy));
    setCopiedRotation(copy);
  };
  const loadCopiedRotation = () => {
    if (!copiedRotation) return;
    updateRotation({ ...copiedRotation, status: "Draft", updatedAt: nowStamp(), submittedAt: "", submittedBy: "Chef" });
  };
  const applyPreview = () => {
    if (!preview) return;
    updateRotation({ ...rotation, ...preview.global, uploadedLtos: preview.uploadedLtos });
    setPreview(null);
  };
    ...
    <div className="mt-4 rounded-3xl border border-sky-200 bg-sky-50/80 p-5 shadow-sm print:hidden">
      <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button onClick={onLoad} disabled={!copiedRotation} className={`rounded-2xl px-5 py-3 text-sm font-semibold ${copiedRotation ? "bg-white border border-sky-200 text-sky-800 hover:bg-sky-100" : "bg-slate-200 text-slate-400 cursor-not-allowed"}`}>Load Copied Rotation</button>
