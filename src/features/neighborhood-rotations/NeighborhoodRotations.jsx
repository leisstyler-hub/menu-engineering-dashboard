...
function isSubmittedRotation(rotation = {}) {
  return rotation.status === "Submitted";
}

function declaredRotationMenuLabel(rotation = {}) {
  if (!isSubmittedRotation(rotation)) return "";
  return rotationMenuLabel(rotation, rotation.cafe, rotation.week) || rotation.menu || "";
}

function hasDeclaredRotationMenu(rotation = {}) {
  return Boolean(declaredRotationMenuLabel(rotation));
}
...
function declaredMenuConflictCounts(rows) {
  return rows.reduce((acc, row) => {
    const menu = declaredRotationMenuLabel(row);
    if (menu) acc[menu] = (acc[menu] || 0) + 1;
    return acc;
  }, {});
}
...
function rowHasDeclaredMenuConflict(row, conflictMenus) {
  const menu = declaredRotationMenuLabel(row);
  if (!menu || !cafeUsesMenuConflictRule(row.district, row.cafe, row.copiedFrom)) return false;
  const count = conflictMenus[`${row.district}|${menu}`] ?? conflictMenus[menu] ?? 0;
  return count > 1;
...
function leadershipRead(rows, conflictMenus) {
  const declared = rows.filter(hasDeclaredRotationMenu).length;
  const missing = rows.filter((row) => !hasDeclaredRotationMenu(row)).map((row) => row.cafe);
  const conflicts = rows.filter((row) => rowHasDeclaredMenuConflict(row, conflictMenus));
...
export default function NeighborhoodRotations({ onBackToPlatform, onOpenSmartsheetHealth }) {
  ...
  const allConflictMenus = Object.entries(DISTRICTS).reduce((acc, [dist, cafeList]) => {
    ...
    Object.entries(declaredMenuConflictCounts(conflictControlledRows(dist, rows))).forEach(([menu, count]) => {
      acc[`${dist}|${menu}`] = count;
    });
    return acc;
  }, {});
        ...
        {rotationView === "planner" && (
          <>
            <section className="grid grid-cols-1 lg:grid-cols-4 gap-4">
              ...
              <StatusCard ready={Boolean(district && selectedCafe)} conflicts={Object.values(conflictMenus).filter((count) => count > 1).length} completed={districtWeekRows.filter(hasDeclaredRotationMenu).length} total={cafes.length} />
            </section>
...
function ExecutiveView({ week, setWeek, rows, conflictMenus }) {
  ...
  const declared = rows.filter(hasDeclaredRotationMenu).length;
  const conflicts = rows.filter((row) => rowHasDeclaredMenuConflict(row, conflictMenus)).length;
  const globalRows = rows.filter(hasDeclaredRotationMenu);
  ...
  const selectedCount = submittedRows.reduce((sum, row) => sum + selectedItems(row).length, 0);
  const districtSignals = districtNames.map((districtName) => {
    ...
    const districtDeclared = districtRows.filter(hasDeclaredRotationMenu).length;
    const districtConflicts = districtRows.filter((row) => rowHasDeclaredMenuConflict(row, conflictMenus)).length;
    const pricedRows = districtRows
      .filter(isSubmittedRotation)
      .map((row) => {
        const range = selectedFoodCostRange(selectedItems(row));
      ...
      <section className="rounded-[2rem] bg-white border-2 border-slate-200 p-6 shadow-2xl">
        ...
        <div className="mt-5 space-y-6">
          {districtNames.map((districtName) => {
            const districtRows = rows.filter((row) => row.district === districtName);
            const districtLocked = districtRows.filter((row) => row.status === "Submitted").length;
            const districtSelected = districtRows.filter(isSubmittedRotation).reduce((sum, row) => sum + selectedItems(row).length, 0);
            return (
              <div key={districtName} className="rounded-3xl border border-slate-200 bg-slate-50/70 p-4">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <p className="rounded-full bg-slate-100 border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600">{districtRows.filter(hasDeclaredRotationMenu).length}/{districtRows.length} declared</p>
                ...
                <div className="mt-3 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                  {districtRows.map((row) => (
                    <SummaryCard key={`${row.district}-${row.cafe}`} row={row} conflict={rowHasDeclaredMenuConflict(row, conflictMenus)} showDistrict={false} submittedOnly />
                  ))}
                </div>
              </div>
            );
...
function LeadershipPulsePanel({ signals, rows, conflictMenus }) {
  ...
  const missingMenuRows = rows.filter((row) => !hasDeclaredRotationMenu(row));
  const duplicateRows = rows.filter((row) => rowHasDeclaredMenuConflict(row, conflictMenus));
  const highCostRows = rows.filter(isSubmittedRotation).filter((row) => {
    const range = selectedFoodCostRange(selectedItems(row));
    const midpoint = range.low != null && range.high != null ? (range.low + range.high) / 2 : null;
  ...
  const actionRows = [
    ...
    { label: "Duplicate menus", value: duplicateRows.length, detail: duplicateRows.slice(0, 4).map((row) => `${row.district}: ${declaredRotationMenuLabel(row)}`).join(", ") || "clear", tone: duplicateRows.length ? "amber" : "green" },
...
function SummaryCard({ row, conflict, showDistrict = true, submittedOnly = false }) {
  ...
  const workingMenuLabel = rotationMenuLabel(row);
  const menuLabel = submittedOnly ? declaredRotationMenuLabel(row) : workingMenuLabel;
  const hasDraftMenu = !locked && Boolean(workingMenuLabel);
  const displayLabel = menuLabel || (submittedOnly && hasDraftMenu ? "Draft saved - not submitted" : "No menu declared");
  ...
  const statusTone = locked ? "bg-emerald-500 text-white border-emerald-500" : hasDraftMenu ? "bg-amber-100 text-amber-900 border-amber-200" : "bg-rose-100 text-rose-900 border-rose-200";
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <div className="flex items-center gap-2">
            <span className={`h-2.5 w-2.5 rounded-full ${locked ? "bg-emerald-500" : hasDraftMenu ? "bg-amber-500" : "bg-rose-500"}`} />
          ...
          <p className="text-base font-black text-slate-950 mt-1 leading-snug">{displayLabel}</p>
        ...
        <div className="flex flex-col items-end gap-2">
          <span className={`rounded-full border px-3 py-1 text-xs font-bold ${statusTone}`}>{locked ? "locked" : hasDraftMenu ? "draft" : "open"}</span>
          {menuLabel && <span className="rounded-full bg-white/80 border border-slate-200 px-3 py-1 text-xs font-bold text-slate-700">{pctRange(fcRange)}</span>}
