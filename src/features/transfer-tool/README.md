...
- The lookup reads direct AP/EP ingredient rows, excludes water and ice, and accepts a price from a canonical `Ingredient:` reference before trying a high-confidence normalized ingredient-and-form match (retaining forms such as chopped, shredded, and sliced). It converts compatible kitchen/metric units and form-limited density measures before calculating ingredient quantity ÷ recipe yield × unit price. When no exact source is available, a substitute must share meaningful food identity, cannot rely on supplier/brand words, and cannot contradict the target's product or preparation type. This prevents failures such as BBQ brisket inheriting beef-pho broth pricing or pepperoncini matching cornichons because both say American Roland. A conservative earlier tier can derive one price from repeated, stable residuals of flat ingredient-only source recipes where every other component is canonically priced; ordinary menu-row portion costs are never treated as an ingredient price directly.

- Approved substitutions: Arcadian Classic Mix (MRN `118307`) uses Spring Mesclun Mix (MRN `3753`) everywhere in the Transfer Tool, with an explicit raw leafy-greens conversion of `1 cup = 1 ounce`; Blue Cheese Crumbles (MRN `1614`) uses the explicitly flagged Cheddar reference MRN `1639` until a canonical blue-cheese price is available. Frank's RedHot Buffalo Wing Sauce (MRN `7179`) uses its exact canonical Snapshot price with the registered product-specific conversion `1 fluid ounce = 1 ounce`. The generic words `classic` and `buffalo` cannot select a substitute source.
- Repeated item names stay isolated by menu, MRN, and portion.
- Selecting a menu item loads its priced ingredient allocation through `/api/transfer-breakdown`. The operator no longer chooses From/To G/L values for priced components; every exported ingredient uses its mapped S4 category code in both S4 G/L fields. The menu card's current Item + Waste Cost is the absolute per-portion ceiling and the visible item/line transfer value. When mapped ingredient cost is above it, every mapped allocation is reduced by the same percentage and the original source allocation remains attached for review. When exactly one recipe component remains unpriced, the authoritative Item + Waste Cost remainder is attributed to that exact component and its existing recipe-mapped G/L, clearly labeled as a residual attribution rather than a Snapshot price. Multiple unresolved components still require one chef-reviewed G/L for the positive remainder. If complete ingredient pricing cannot produce a valid allocation because the mapping endpoint explicitly returns 404/no mapping or unresolved components remain after mapped prices consume the full cost, the approved fallback replaces partial rows with one visibly labeled `4111011 — Prepared Foods` row for the complete Item + Waste Cost. A finite positive Item + Waste Cost is mandatory; the fallback never invents a cost. A network, malformed-response, or 5xx failure remains blocked rather than being treated as an absent mapping. Client and server validation enforce the single-row fallback shape and exact equality with Item + Waste Cost.
...
- One selected item normally expands into one S4 row per ingredient: its mapped G/L code in both S4 G/L columns, receiving profit center, an automatic item-and-ingredient description capped at 50 characters, item count × ingredient allocation per portion rounded to two decimals, and the transfer Event ID (18 characters). An approved Prepared Foods fallback exports exactly one `4111011` row instead. Per-row cent rounding is reconciled within the item so the exported row sum equals item count × Item + Waste Cost exactly and can never exceed it.
- Each transfer line shows one compact, collapsed allocation summary by default. Expanding it reveals every ingredient, source MRN, G/L, substitute/residual note, original pre-cap cost, proportional adjustment, unresolved component, and reconciliation total. While a positive remainder needs review, the compact amber G/L selector and reconciliation guidance remain visible. After the chef selects a G/L, those review controls and guidance disappear and every completed mapped allocation row receives a light-green approved treatment.
