...
- The lookup reads direct AP/EP ingredient rows, excludes water and ice, and accepts a price from a canonical `Ingredient:` reference before trying a high-confidence normalized ingredient-and-form match (retaining forms such as chopped, shredded, and sliced). It converts compatible kitchen/metric units and form-limited density measures before calculating ingredient quantity ÷ recipe yield × unit price. When no exact source is available, a name-anchored, unit-convertible Ingredient Snapshot substitute may be used and is explicitly flagged for chef review; generic category matches, numeric/cut-form-only matches, and flavor-only matches such as sauce-to-sauce or buffalo chicken-to-buffalo sauce are rejected. A conservative earlier tier can derive one price from repeated, stable residuals of flat ingredient-only source recipes where every other component is canonically priced; ordinary menu-row portion costs are never treated as an ingredient price directly.
...
- Selecting a menu item loads its priced ingredient allocation through `/api/transfer-breakdown`. The operator no longer chooses From/To G/L values for priced components; every exported ingredient uses its mapped S4 category code in both S4 G/L fields. The menu card's current Item + Waste Cost is the absolute per-portion ceiling and the visible item/line transfer value. When mapped ingredient cost is above it, every mapped allocation is reduced by the same percentage and the original source allocation remains attached for review; the final allocation reconciles exactly to Item + Waste Cost. When mapped cost is lower, the chef must choose one approved G/L for the positive remainder; that selection is the review approval and creates one clearly labeled residual S4 line. Save/export are blocked until the choice is made. The server independently rejects any saved S4 transfer whose allocations do not equal Item + Waste Cost.
