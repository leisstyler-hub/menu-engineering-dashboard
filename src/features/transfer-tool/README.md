...
- The lookup reads direct AP/EP ingredient rows, excludes water and ice, and accepts a price only from a canonical `Ingredient:` reference. It tries the exact ingredient MRN first, then a high-confidence normalized ingredient-and-form match (retaining forms such as chopped, shredded, and sliced), and converts compatible units before calculating ingredient quantity ÷ recipe yield × that unit price. Ordinary menu-row portion costs are never treated as ingredient prices.
