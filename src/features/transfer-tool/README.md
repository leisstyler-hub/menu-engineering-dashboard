...
- The lookup reads direct AP/EP ingredient rows, excludes water and ice, and accepts a price from a canonical `Ingredient:` reference before trying a high-confidence normalized ingredient-and-form match (retaining forms such as chopped, shredded, and sliced). It converts compatible kitchen/metric units and form-limited density measures before calculating ingredient quantity ÷ recipe yield × unit price. When no exact source is available, a name-anchored, unit-convertible Ingredient Snapshot substitute may be used and is explicitly flagged for chef review; generic category matches such as `oil` or `sauce` are rejected. A conservative earlier tier can derive one price from repeated, stable residuals of flat ingredient-only source recipes where every other component is canonically priced; ordinary menu-row portion costs are never treated as an ingredient price directly.

- Approved substitution: Arcadian Classic Mix (MRN `118307`) uses Spring Mesclun Mix (MRN `3753`) everywhere in the Transfer Tool, with an explicit raw leafy-greens conversion of `1 cup = 1 ounce`. This is displayed to the chef as a substitute price; the generic word `classic` cannot select a price source.
- Repeated item names stay isolated by menu, MRN, and portion.
- Selecting a menu item loads its priced ingredient allocation through `/api/transfer-breakdown`. The operator no longer chooses From/To G/L values. Every exported ingredient uses its mapped S4 category code as both G/L values. If a recipe still has an unpriced component but the current Item + Waste Cost is higher than its priced allocation, the positive difference is exported as a clearly labeled `Cost balance adjustment` to `4111011 Prepared Foods`; the unresolved source component remains visible to the chef. A selection without a positive priced allocation or a positive balance target cannot save or export; older saved records remain visible but need to be copied and reselected before export.
