...
- The lookup reads direct AP/EP ingredient rows, excludes water and ice, and uses the first direct one-unit observation for the matching ingredient MRN/unit as its standardized price basis. Each allocation is ingredient quantity ÷ recipe yield × that unit price.
