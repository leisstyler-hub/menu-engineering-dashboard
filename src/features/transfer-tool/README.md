...
- Selecting a menu item loads its priced ingredient allocation through `/api/transfer-breakdown`. The operator no longer chooses From/To G/L values for priced components; every exported ingredient uses its mapped S4 category code in both S4 G/L fields. The menu card's current Item + Waste Cost is the visible item/line transfer value, while the ingredient list is the per-portion G/L allocation. When that allocation is lower than the menu cost, the chef must choose one approved G/L for the positive remainder; that selection is the review approval and creates one clearly labeled residual S4 line. Save/export are blocked until the choice is made. An allocation above Item + Waste Cost remains blocked for source review; no negative correction is created.
