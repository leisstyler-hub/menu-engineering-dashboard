...
- [Sep 3, 10:45 AM] Prepared Tyler's urgent SSMT load-resilience, Supabase slow-path, and editable-menu-name fix as version `2026.09.03.002-ssmt-load-resilience`. Live investigation reproduced the production symptom: `GET /api/storage/records?tool=SSMT&includeHidden=1` returned a 522 after about 20 seconds from the workspace Node fetch path after Tyler's Supabase Pro upgrade, `GET /api/recipe-library?scope=summary` returned only after about 39 seconds with the MenuWorks fallback source, and Vercel runtime logs showed repeated `/api/traffic/weekly` Supabase calls failing around 19-20 seconds. Root cause found in code: SSMT and Menu Library waited on Supabase reads without timeouts, Recipe Library serialized its normal recipe read and SSMT workspace read, the SSMT client still auto-saved the full shared workspace after every fast edit burst despite the new manual save buttons, and page traffic analytics could wait on slow Supabase reads/writes. The candidate adds server-side Supabase request timeouts for storage and Recipe Library reads so slow calls return fallback/error state instead of hanging while allowing a longer write window for shared saves; runs Recipe Library's normal Supabase row read and SSMT operating-row read concurrently; compresses the single shared `ssmt|workspace|current` payload before writing it to indexed `app_records.record_payload` and expands it again on storage/Recipe Library reads; saves the existing SSMT shared workspace row through a direct one-row `PATCH` instead of the slower upsert-conflict path; adds client-side shared SSMT load/save timeouts so SSMT falls back to local cache/seed quickly; changes SSMT shared autosave to a longer idle save with duplicate-payload suppression while preserving immediate local cache and manual Save buttons; puts short Supabase timeouts around the weekly traffic endpoint so analytics degrades instead of tying UI activity to 20-second Supabase waits; and adds an editable `Menu name` field in the selected-menu editor. Renamed menus update the selector, downstream SSMT-derived preview, shared workspace save payload, and Centric export filename. Added `scripts/verify-ssmt-load-resilience.mjs` and wired it into `pnpm run verify`. Local proof covers concurrent fallback from slow Supabase reads, compressed SSMT workspace save/read behavior, direct one-row SSMT shared-workspace patching, fast traffic fallback, and editable menu names. No Supabase schema change, manual production-data write, destructive Menu Library/App/Webtrition deletion, Webtrition Report Menu Index removal-authority change, pricing authority change, or MRN coercion is included. Release state: IMPLEMENTED.
