...
- [Sep 7, 2:57 PM] Added a `Scan & Pay` column to the SSMT Menu Builder item table so IT-generated UPC/barcode numbers can be stored on each item. The new column sits between `Secondary category` and `Area prices` in the builder header, colgroup, and each item row, and renders a monospace text input (placeholder `UPC`) bound to a new per-item `scanPayUpc` field. It follows the same interaction pattern as the sibling text fields (MRN, Category, Secondary category): editable while the item is unlocked, and click-to-copy while the item is locked for Centric via the existing `copyLockedField` helper. The field is initialized to `""` in `createBlankItem` and normalized in `cloneMenu`, so it round-trips through the existing SSMT workspace save/load exactly like the other item text fields with no new persistence path. The divider/sub-menu row `colSpan` was bumped from 10 to 11 to span the added column; the builder table's `min-w` was left at `2680px` (the sum of explicit column widths is 2341px, well under the lock) so the maximized-desktop width test at `tests/browser/ssmt-tool.spec.js:334` still passes with no horizontal scroll. This is a UI/data-entry additive change only — no Supabase schema change, manual production-data write, destructive deletion, source-authority change, pricing authority change, modifier-data change, MRN coercion, or downstream SSMT-to-Menu-Library wiring (which remains unauthorized). Added browser coverage in `tests/browser/ssmt-tool.spec.js`: the builder-columns test now asserts the `Scan & Pay` header is visible and that its input accepts and retains a UPC value. Evidence: full SSMT browser suite 17/17 via `node scripts/run-playwright.mjs tests/browser/ssmt-tool.spec.js`; `corepack pnpm run build` clean; full `corepack pnpm run verify` passed (exit 0). Set visible app version to `2026.09.07.004-ssmt-scan-pay-column`. Release state: LIVE.

- [Sep 7, 1:52 PM] Added a sixth `Hibernated` bucket to the SSMT Menu Selector, stacked the area-price cells airport-code-over-price, and made a locked price copy just the number. Before this pass, turning on `Show hibernated` left hibernated/expired menus mixed into their normal type buckets (Core/Global/etc.), so there was no single place to see everything that was hibernated; `groupMenusByType` in `src/features/ssmt/SsmtTool.jsx` now pulls every hidden or auto-expired menu out of the five type buckets and into a dedicated sixth `Hibernated` bucket that only appears while `Show hibernated` is on, and dropping a menu onto that bucket hibernates it (instead of corrupting its type). The five active type buckets and default (hibernated-off) view are unchanged. Second, each area-price cell in the item builder and in the modifier detail grid now stacks the airport code on top of the price (for example `AUS` over `$12.10`) instead of showing them inline on one line, keeping the same 8-and-8 two-row layout and the same area order. Third, clicking a locked item's area price to copy it for Centric now copies just the number and decimal point (`12.10`) via a new `priceDigitsOnly` helper that strips the `$` and any grouping characters; the on-screen price still shows the `$` for readability, and non-price locked fields (name, MRN, description, category, calories) are unchanged. This is a UI/interaction change only — no Supabase schema change, manual production-data write, destructive Menu Library/App/Webtrition deletion, source-authority change, pricing authority change, modifier-data change, or MRN coercion. Added browser coverage in `tests/browser/ssmt-tool.spec.js`: "SSMT collects hibernated menus into a sixth bucket shown on demand" (hibernates a fresh menu, proves it leaves the Core bucket and appears in a sixth `Hibernated` bucket only when `Show hibernated` is on, and that the bucket sits last), and the existing lock/copy test now asserts the displayed price keeps `$` while the copied clipboard value is `$`-stripped; the area-price parsing assertion was updated for the new stacked markup. Evidence: full SSMT browser suite 17/17 via `node scripts/run-playwright.mjs tests/browser/ssmt-tool.spec.js`; `npm run build` clean; full `npm run verify` passed (exit 0). Note: the version stamp is intentionally kept concise (`2026.09.07.003-ssmt-hibernate-bucket-and-prices`) because the visible header wraps the stamp onto a second line when it is long, which shifts the whole page down and trips the borderline viewport-density test at `tests/browser/ssmt-tool.spec.js:276`. Set visible app version to `2026.09.07.003-ssmt-hibernate-bucket-and-prices`. Release state: LIVE.

- [Sep 7, 8:27 AM] Gave SSMT dividers and sub menus rotating colors and made a second Flag click on an already-flagged item offer edit-or-clear. Before, every divider row (and its items) rendered violet and every sub menu row (and its items) rendered emerald, so two dividers on the same menu looked identical and items were not organized by section color. Dividers now rotate through four colors (violet → rose → amber → cyan) and sub menus rotate through their own four (emerald → indigo → fuchsia → orange), cycling 1-2-3-4-1… independently within a single menu; each divider/sub menu's items inherit that section's color so sections read by color. The first divider stays violet and the first sub menu stays emerald, preserving the prior look for the common single-section case. Implemented as literal Tailwind palette tables (`SSMT_DIVIDER_PALETTES`, `SSMT_SUBMENU_PALETTES`, `SSMT_BUILDER_MAIN_PALETTE`) in `src/features/ssmt/SsmtTool.jsx`, with the builder row loop tracking a per-kind color index and the active palette so items below a section header pick up its tone. Separately, clicking `Flag` on an item that already has a saved flag now opens a "This item is already flagged" prompt with `Edit flag` / `Clear flag` / `Cancel` instead of silently stacking a second flag: Edit reopens the flag dialog in update mode (title `Edit item flag`, button `Update flag`) and replaces the existing flag in place rather than appending a duplicate; Clear removes just that item's flag and resets the reported-flag banner when none remain. The Flag button on a flagged item now reads `Flagged` with a filled amber style and an "Edit or clear flag for …" label. Flags remain report-only and menu-session-scoped; no email is sent by editing or clearing. This is a UI/interaction change only — no Supabase schema change, manual production-data write, destructive deletion, source-authority change, pricing authority change, modifier-data change, or MRN coercion. Added browser coverage in `tests/browser/ssmt-tool.spec.js`: "SSMT dividers and sub menus rotate through distinct colors on one menu" (asserts divider 1 violet-400 vs divider 2 rose-400, sub menu 1 emerald-400 vs sub menu 2 indigo-400) and "SSMT second flag click on a flagged item prompts edit or clear" (proves the edit-or-clear prompt, in-place update, and clear). Evidence: full SSMT browser suite 16/16 via `node scripts/run-playwright.mjs tests/browser/ssmt-tool.spec.js`; `npm run build` clean; full `npm run verify` passed. Set visible app version to `2026.09.07.002-ssmt-section-colors-and-flag-edit`. Release state: LIVE.

- [Sep 7, 8:13 AM] Added a `Clear flags` button next to `Report flags` on the SSMT selected-menu editor so a user can manually clear the saved report-only flags on a menu without having to leave the menu. Before this pass the only way to clear flags was to switch to a different menu and confirm the "Exiting this menu with unreported flags" leave prompt, so Tyler had no in-place way to clear flags he no longer wanted to report. The new button (rendered only when the menu has at least one saved flag) opens a small confirm modal — "Clear all N saved flags on {menu}? … use Report flags first if you still need to email them. This cannot be undone." — and on confirm empties `flags` on the selected menu via the existing `updateSelectedMenu({ flags: [] })` path and resets the last-reported banner, identical to the clear behavior the leave prompt already performs. Flags remain report-only and menu-session-scoped: clearing sends nothing and only removes the local saved flags. This is a UI-only additive change — no Supabase schema change, manual production-data write, destructive Menu Library/App/Webtrition deletion, source-authority change, pricing authority change, modifier-data change, or MRN coercion is included. Evidence: `npm run build` clean; `npx playwright test tests/browser/ssmt-tool.spec.js` passed 13/13, with the single pre-existing viewport-density layout failure at `tests/browser/ssmt-tool.spec.js:276` (`y+height <= 900`) confirmed failing identically on the clean base commit `fe76ffb`, unrelated to this diff. Set visible app version to `2026.09.07.001-ssmt-clear-flags-button`. Release state: LIVE.

## 2026-09-06

- [Sep 6, 12:24 AM] Applied the same exact-match modifier logic to the SSMT Centric export that the `2026.09.05.004` badge fix applied to the in-app `Mods (N)` badge, and fixed the three generated-data `--check` guards that were failing on every Windows checkout. Before this pass, `relatedModifierGroups` in `src/features/ssmt/ssmtCentricExport.js` still matched an item's free-text `modifierGroups` refs to master groups by loose substring, so template-artifact groups left by the original seed import (group names like `Forced/Add/Remove?`, `NO`, `1`, `0`, `FORCE`) could silently attach to unrelated items and inject fabricated modifier rows plus literal placeholder text (`Choices`, `description`, `MRN`, `Price`) into the exported workbook that Tyler said "IT will just click export" and trust. `relatedModifierGroups` now matches only on exact group id (how freshly built groups are linked) or exact case-insensitive group name, identical to `modifierGroupMatchesItemRef` in `SsmtTool.jsx`, so the export and the in-app badge/dialog now agree. Added a regression to `scripts/verify-ssmt-centric-export.mjs` proving template-artifact group names can no longer substring-match an item into the export and that placeholder choice text never reaches the Modifiers sheet; the new assertion was confirmed to fail on the old loose logic and pass on the fix. Separately, the three build-time drift guards `build-food-cost-plate-reference.mjs --check`, `build-menu-cross-utilization-data.mjs --check`, and `build-ssmt-seed-data.mjs --check` were all failing on Windows checkouts — the pre-existing "stale generated food-cost reference" blocker referenced in several recent entries that stopped full `verify` before it reached SSMT. Root cause was not real data drift: each guard compared its freshly generated LF output against the committed artifact read back as CRLF (Git `core.autocrlf` converts the committed LF files on Windows checkout), so the byte comparison always failed. The three guards now normalize CRLF to LF before comparing, so they test real content drift rather than the platform's line-ending conversion. `build-menu-cross-utilization-data.mjs` had a second, independent non-determinism: it stamped `generatedAt` with `new Date()` at build time, guaranteeing the guard would go stale whenever the wall clock rolled to a new day; it is now pinned to `2026-08-23` (its current committed value) to match the deterministic pattern already used by the SSMT seed builder, to be bumped manually when the underlying source data actually changes. No generated data file was regenerated or changed — `public/data/ssmtSeedData.json`, `src/data/menuCrossUtilization.json`, and the food-cost reference are byte-identical to before this pass, and no modifier data was wiped or altered. With these fixes the full `npm run verify` chain now completes clean through `vite build` for the first time in weeks instead of stopping at the food-cost guard. Evidence: `node scripts/verify-ssmt-centric-export.mjs` passes with the new regression; all three `--check` guards pass; full `npm run verify` completes clean; `node scripts/run-playwright.mjs tests/browser/ssmt-tool.spec.js` passed 14/14 including the Centric export and badge-count tests; the 14 unrelated browser failures in the full suite (rotations, reinvent-recall, recipe-library, landing-changelog) were confirmed pre-existing on the clean base commit `da8f91e`. No Supabase schema change, manual production-data write, destructive deletion, source-authority change, pricing authority change, or MRN coercion is included. Set visible app version to `2026.09.06.001-ssmt-export-mods-match-and-check-guards`. Release state: LIVE.

## 2026-09-05

- [Sep 5, 7:09 PM] Corrected the SSMT `Mods (N)` badge so it counts only modifier groups that are really linked to an item, and bundled the pending SSMT builder layout follow-up. Previously the badge showed the raw length of an item's free-text `modifierGroups` refs, and the modifier dialog attached any master group whose name was a loose substring of those refs. Because the original seed import populated the master modifier-group list almost entirely with template-sheet artifacts (group names like `Forced/Add/Remove?`, `NO`, `1`, `0`, `2`, `FORCE`), items such as Andes → `PERUVIAN STEWED TOFU` showed `Mods (3)` while the dialog was empty or showed fabricated template rows. `modifierGroupMatchesItemRef` now matches only on exact group id (how freshly built groups are linked) or exact case-insensitive group name, and the badge count is derived from that same matched set the dialog renders, so the number always equals what opens. Across the current seed this drops 1,301 items that showed false counts down to the single item with a genuinely authored link, while freshly built groups (linked by id) still count correctly. This pass also ships the pending builder UI work from branch commit `32d0119`: Area Prices now render 8 on top / 8 on bottom (was 10/6) in original order, the `Mods` button sits left of the `Lock` button and both are larger, the builder surface is slightly wider, the modifier dialog's locked Name/Description/MRN/Calories fields are now click-to-copy for IT (they were only read-only before), and leaving a menu that still has unreported flags now prompts `Exiting this menu with unreported flags. Do you wish to continue?` and clears the report-only flags on confirm. No modifier data was deleted or wiped, and there is no Supabase schema change, manual production-data write, destructive deletion, source-authority change, pricing authority change, or MRN coercion — the modifier data itself is unchanged and Culinary rebuilds groups fresh in-app. The Centric export's separate modifier-matching path was intentionally left unchanged in this pass. Added browser coverage `tests/browser/ssmt-tool.spec.js` "SSMT Mods badge counts only reliably-linked groups, not stale free-text refs" proving the Andes tofu item reads `Mods (0)` with an empty dialog and that a freshly added group makes the badge read `Mods (1)`; local `npx playwright test tests/browser/ssmt-tool.spec.js` passed 14/14 and `npm run build` was clean. Set visible app version to `2026.09.05.004-ssmt-mods-count-and-builder`. Release state: LIVE.

- [Sep 5, 5:46 PM] Matched SSMT Menu Builder item rows to their parent visual section and restored full Area Prices visibility. Main Menu Items now carry the blue row/border treatment, items under Sub Menu rows carry the green treatment, and items under Divider rows carry the violet treatment; Area Prices now renders every configured area in the builder and modifier detail instead of relying on a collapsed `+N` preview. Added browser coverage for section-tone inheritance and for all 16 area labels being visible with no `+N` badge. This is UI-only and does not change SSMT data, save, export, pricing values, source-authority, Supabase, Webtrition/MenuWorks, MRN, modifier data, deletion, or submit behavior. Set visible app version to `2026.09.05.003-ssmt-section-price-display`. Release state: LIVE.

- [Sep 5, 10:17 AM] Matched SSMT Menu Builder item rows to their parent visual section. Main Menu Items now carry the blue row/border treatment, items under Sub Menu rows carry the green treatment, and items under Divider rows carry the violet treatment so the rows visually belong to the section above them. Added browser coverage for the section-tone inheritance. This is UI-only and does not change SSMT data, save, export, pricing, source-authority, Supabase, Webtrition/MenuWorks, MRN, modifier, deletion, or submit behavior. Set visible app version to `2026.09.05.002-ssmt-section-row-colors`. Commit `f70a9b6` was pushed to GitHub `main`; superseded by `2026.09.05.003-ssmt-section-price-display` before final live proof because the built-in live checker hit the known Windows receive error.
...
- [Sep 4, 10:57 PM] Hid the non-functioning MenuWorks Truth Upload panel from the Menu Library screen per Tyler while SSMT moves toward being the single source of truth for menus/items. Published to GitHub `main`: the panel gate shipped in application commit `2c1e480` (merge `a27e92b`) and this follow-up corrects the version-date typo `2026.09.05.001` back to the platform's Pacific-Time convention `2026.09.04.001-hide-menuworks-truth-upload`. The panel is gated behind a new `SHOW_MENUWORKS_TRUTH_UPLOAD` constant (false) in `src/features/recipe-database/RecipeDatabase.jsx`; the `MenuWorksRefreshPanel` component and the entire MenuWorks import pipeline (initiation code, file parse, review modal, and the acceptMenuWorksImport action) are intentionally left intact so this upload slot can return as the Webtrition item-index "empty bucket" fill once SSMT is up to date. The application diff is 9 insertions / 1 deletion across two files. `corepack pnpm run build` passed and the live app served the `2026.09.04.001-hide-menuworks-truth-upload` version stamp after deploy. No Supabase schema change, manual production-data write, destructive deletion, MenuWorks/Webtrition import-authority change, pricing authority change, or MRN coercion is included. Release state: LIVE.

- [Sep 4, 7:19 AM] Published Tyler's SSMT shared-workspace record-id reconciliation to GitHub `main` at merge commit `547e15c2ba5f7fd1c6958508de6ef710a1004ff6` with application commit `2b294c11bf215072ee5cf0c0aa4c9c5089aae9ce` as version `2026.09.03.003-ssmt-workspace-record-id-reconcile`. This release reconciles all SSMT shared-storage reads and writes back to the live `ssmt|workspace|current` record id after `2026.09.03.002-ssmt-load-resilience` redirected the shared workspace and Recipe Library SSMT row path toward `ssmt|workspace|current-v2`, a compact-successor record that was never created. A fresh browser with no local cache could therefore load an empty shared SSMT workspace and then shadow the real data on first save. The shipped `.003` code restores `current` as the single write target while still accepting `current-v2` defensively on read, preserving the gzip-compacted single-row workspace flow without any schema or data migration. Independent September 4 production checks confirmed GitHub `main` at `547e15c`, live HTML referencing `/assets/index-BwVQwATB.js`, that deployed asset carrying the `.003` version stamp, `/api/storage/records?tool=SSMT&includeHidden=1` returning HTTP 200 in about 1.3 seconds with one SSMT workspace payload containing 104 menus, and `/api/recipe-library?scope=summary` returning `source: supabase-recipe-items+ssmt-derived`, `ssmtDerivedRows: 5`, and no fallback message. Local release evidence from the merged candidate passed `node scripts/verify-ssmt-load-resilience.mjs`, `node scripts/verify-ssmt-derived-menu-source.mjs`, and `corepack pnpm run build`; local `node scripts/run-playwright.mjs tests/browser/ssmt-tool.spec.js` remained 11/12 with only the pre-existing viewport-density assertion at `tests/browser/ssmt-tool.spec.js:287` (`<= 900`) failing identically on clean `HEAD`. No Supabase schema change, manual production-data write, destructive Menu Library/App/Webtrition deletion, Webtrition Report Menu Index removal-authority change, pricing authority change, or MRN coercion is included. Release state: LIVE.

## 2026-09-03

- [Sep 3, 10:45 AM] Deployed Tyler's urgent SSMT load-resilience, Supabase slow-path, and editable-menu-name fix as version `2026.09.03.002-ssmt-load-resilience`; release state is `DEPLOYED`, not `LIVE`, because production SSMT verification still shows shared Supabase saves timing out. Live investigation reproduced the production symptom: `GET /api/storage/records?tool=SSMT&includeHidden=1` returned a 522 after about 20 seconds from the workspace Node fetch path after Tyler's Supabase Pro upgrade, `GET /api/recipe-library?scope=summary` returned only after about 39 seconds with the MenuWorks fallback source, and Vercel runtime logs showed repeated `/api/traffic/weekly` Supabase calls failing around 19-20 seconds. The deployed fix adds server-side Supabase request timeouts for storage and Recipe Library reads so slow calls return fallback/error state instead of hanging while allowing a bounded write window for shared saves; runs Recipe Library's normal Supabase row read and SSMT operating-row read concurrently; writes normal SSMT saves to a compact successor record `ssmt|workspace|current-v2` instead of repeatedly touching the oversized legacy `ssmt|workspace|current` row; compresses the v2 workspace payload before writing it to `app_records.record_payload` and expands it again on storage/Recipe Library reads; adds client-side shared SSMT load/save timeouts so SSMT falls back to local cache/seed quickly; changes SSMT shared autosave to a longer idle save with duplicate-payload suppression while preserving immediate local cache and manual Save buttons; puts short Supabase timeouts around the weekly traffic endpoint so analytics degrades instead of tying UI activity to 20-second Supabase waits; and adds an editable `Menu name` field in the selected-menu editor. Renamed menus update the selector, downstream SSMT-derived preview, shared workspace save payload, and Centric export filename. Production deployment `dpl_BraieLUBNwNGnwdT9N9M9V5JUmJm` reached `READY` at commit `b63a2c9a3185aeee43ddadd6f6a608da3765803d`, and production served `/assets/index-BRcAcGaw.js` containing the `.002` version stamp. Direct production API checks now fail/recover much faster than the reported three-minute load: SSMT storage returns 504 fallback in about 8.3s, Recipe Library returns MenuWorks fallback in about 8.4s, and weekly traffic returns degraded fallback in about 2.7s. Production SSMT Playwright still failed 1/12 because shared save returned `Supabase storage request timed out`; local SSMT Playwright passed 12/12. No Supabase schema change, manual production-data write, destructive Menu Library/App/Webtrition deletion, Webtrition Report Menu Index removal-authority change, pricing authority change, or MRN coercion is included. Release state: DEPLOYED.

- [Sep 3, 7:29 AM] Published Tyler's SSMT manual-save, modifier clipboard, item flag report, and row-contrast follow-up to GitHub `main` at application commit `bc16d46a521f107564d38b84264f92137757aaf7` and release-record commit `7887de9cf8653fdbdee00c4ba9bcb7ca3fe56367` as version `2026.09.03.001-ssmt-manual-save-flags`. GitHub's Vercel status for the release-record commit reported success with `Deployment has completed` and target `https://vercel.com/tylerl-s-projects/project-d8v25/719mdkdzycKp5ihHKZWX5o6pc3pj`. Production asset checks found the live version stamp, manual-save markers, modifier clipboard markers, flag report timestamp markers, and Centric export markers in the deployed app. SSMT now exposes explicit `Save SSMT workspace`, `Save menu`, and `Save modifiers` actions that immediately flush the current SSMT workspace through the existing shared `ssmt|workspace|current` storage record and show a retry-ready error message if shared storage rejects the save while retaining the browser cache. Modifier groups now use four visible clipboard slots; users can save a modifier group into slot 1-4, see which slots are empty or named, and paste any populated slot onto another item as an independent group. Item flags are now saved onto the selected SSMT menu with reason, note, item name, and timestamp; each menu shows its saved flag count and generates a `Report flags` mail draft to `alexander.neuse@compass-usa.com` and `tyler.leiss@compass-usa.com` containing all saved flags for that menu plus a report timestamp. Menu item rows and modifier rows now use darker row borders and stronger alternating fills for easier scanning. The existing selected-menu Centric export remains `[Brand Name] SSMT Export.xlsx` with the eight approved Centric-shaped tabs; the focused export verifier still passes. This is an app-side SSMT workflow/storage-payload change only: no Supabase schema change, no manual production-data write, no destructive Menu Library/App deletion, no Webtrition Report Menu Index removal-authority change, no pricing authority change, and no MRN coercion. Local TDD proof first failed on missing manual-save UI and old row contrast, then passed `node scripts/run-playwright.mjs tests/browser/ssmt-tool.spec.js` 11/11. Focused local guards passed `node scripts/test-storage-backbone.mjs`, `node scripts/verify-ssmt-centric-export.mjs`, `node scripts/build-ssmt-seed-data.mjs --check`, `node scripts/verify-ssmt-derived-menu-source.mjs`, `node scripts/verify-changelog-timestamps.mjs`, `node scripts/verify-release-workflow.mjs`, `node scripts/verify-platform-tools.mjs`, `node scripts/verify-menu-audit-tool.mjs`, `git diff --check`, and `corepack pnpm run build`. Production browser proof passed `node scripts/run-playwright.mjs tests/browser/ssmt-tool.spec.js` 11/11 against `https://project-d8v25.vercel.app`. Full `corepack pnpm run verify` still stops before SSMT at the pre-existing stale generated food-cost reference guard. Release state: LIVE.

## 2026-08-31

- [Aug 31, 7:22 PM] Published Tyler's Phase 2 SSMT source-authority bridge to GitHub `main` at application commit `0d383bc80abd78939c96fdfd6af713a1c9110394` as version `2026.08.31.005-ssmt-derived-menus`. Live production HTML references `/assets/index-1LZOfrzn.js`, and that asset contains the `.005` version stamp. SSMT selected-menu rows now support dark, draggable, deletable `Sub menu` divider rows; items below a sub menu divider are grouped under that downstream sub menu until the next sub menu divider. IT-complete, non-hidden `Core`, `Global`, and `Menu Library` SSMT menus now derive Menu Library / Neighborhood Rotation selector rows from the shared `ssmt|workspace|current` workspace through the existing `/api/recipe-library` read path. SSMT-owned downstream menu names replace matching MenuWorks rows at read time only, so stale source rows do not appear for those menus while unrelated MenuWorks menus remain available. Menu Library labels the combined source as `Menu data + SSMT`, and Neighborhood Rotations now describes the loader as generic menu selector data because the same API can include SSMT-derived rows. Production API check returned `source: supabase-recipe-items+ssmt-derived`, `ssmtDerivedRows: 2`, and no fallback message. Added `node scripts/verify-ssmt-derived-menu-source.mjs`, SSMT browser coverage for sub menu creation, dark-row styling, drag placement, downstream preview, and deletion, and Neighborhood Rotations browser coverage proving SSMT-derived submenu rows can appear in the downstream menu selector. Local proof passed `node scripts/verify-ssmt-derived-menu-source.mjs`, `node scripts/verify-menu-audit-tool.mjs`, `node scripts/verify-recipe-library-backbone.mjs`, `node scripts/verify-rotation-integrity.mjs`, `node scripts/build-ssmt-seed-data.mjs --check`, `node scripts/verify-platform-tools.mjs`, `node scripts/verify-menu-projects.mjs`, `node scripts/verify-ssmt-centric-export.mjs`, `node scripts/verify-release-workflow.mjs`, `node scripts/verify-changelog-timestamps.mjs`, `git diff --check`, `corepack pnpm run build`, full local `node scripts/run-playwright.mjs tests/browser/ssmt-tool.spec.js` 10/10, and focused local `node scripts/run-playwright.mjs tests/browser/neighborhood-rotations.spec.js --grep "SSMT-derived submenus"` 1/1. Production browser proof passed full `tests/browser/ssmt-tool.spec.js` 10/10 and the focused Neighborhood Rotations SSMT-derived submenu test 1/1 against `https://project-d8v25.vercel.app`. Full `corepack pnpm run verify` still stops before SSMT at the pre-existing stale generated food-cost reference guard. The full Menu Library browser suite still has the pre-existing `data-library-property-label` count mismatch, and the full Neighborhood Rotations browser suite now has date-sensitive July 2026 week-option / old plate-cost pilot failures because the local date is August 31, 2026. No Supabase schema change, manual production-data write, destructive Menu Library/App deletion, Webtrition Report Menu Index removal-authority change, pricing authority change, or MRN coercion is included. Release state: LIVE.

- [Aug 31, 1:36 PM] Published Tyler's SSMT modifier workflow, export cleanup, pricing, menu-selector status, hibernation, and legacy shared-workspace bucket correction follow-up to GitHub `main` at release head commit `6dd555cbc814bbdd531978f459dbb97fc7e78756` as version `2026.08.31.004-ssmt-legacy-bucket-correction`. Live production HTML references `/assets/index-DG3Hcib6.js`, and that asset contains the `.004` version stamp plus the SSMT workflow markers. Centric exports now leave optional unknown business fields blank instead of `[placeholder]` while preserving blank Centric-owned ID columns and exact MRN text. New item rows visibly start at `Mods (0)` and opening their modifier detail no longer shows fallback seed groups; adding a modifier group creates one modifier item line. Modifier item text normalizes to lower case, modifier entry fields are lighter-weight for readability, the modifier dialog is wider, modifier groups can be locked/unlocked, copied to an SSMT clipboard, pasted onto another item as an independent group, and drag-reordered. Menu deletion again requires retyping the exact menu name, while item/modifier deletes keep the faster app-side confirmation. Pricing Structure rows now expose editable area-price fields plus a modifier-only checkbox; item selectors sort non-modifier prices high-to-low before modifier-only rows, modifier selectors prioritize modifier-only rows, and a new `Price Category Modifier Free` row is forced to `0.00` across all areas. The menu selector now has a fifth `Menu Library` bucket, drag/drop movement between buckets, an IT edit-flag callout, status counters under the selector, and hibernate/restore controls with expired Promotion menus hidden from the main view unless hibernated menus are shown. Older shared SSMT workspace records now load with missing seed menus appended and legacy seed-menu bucket corrections applied once, then save a `seedMenuTypeCorrectionsApplied` flag so later manual drag/drop bucket moves persist. Production DOM inspection showed the corrected Global bucket including `Andes`; production Playwright passed `node scripts/run-playwright.mjs tests/browser/ssmt-tool.spec.js` 10/10 against `https://project-d8v25.vercel.app`. Local focused proof passed `node scripts/test-storage-backbone.mjs`, `node scripts/verify-ssmt-centric-export.mjs`, `node scripts/build-ssmt-seed-data.mjs --check`, `git diff --check`, `corepack pnpm run build`, and `node scripts/run-playwright.mjs tests/browser/ssmt-tool.spec.js` 10/10. Full `corepack pnpm run verify` remains blocked before SSMT by the pre-existing stale generated food-cost reference guard. This release does not implement the broader Phase 2 source-authority bridge where SSMT deletes Menu Library/App items or where Menu Library and Neighborhood Rotations consume SSMT submenus; no Supabase schema change or manual production-data rewrite is included. Release state: LIVE.

- [Aug 31, 8:32 AM] Published Tyler's SSMT shared-workspace persistence, Centric export, and menu-list grouping follow-up to GitHub `main` at application commit `7a248dbef7804b2078af3375f89df495467f0bb1` as version `2026.08.31.001-ssmt-centric-export`. Vercel production deployment `dpl_4a6TZZzKoFqiC6c822a7Hq6bybJ9` is `READY`; live HTML references `/assets/index-DY-te2V0.js`, that live index contains the version stamp, and the live SSMT chunk `/assets/SsmtTool-DewrBY4k.js` contains the export and shared-workspace markers. The selected-menu item-lines screen now has an `Export SSMT` button that downloads `[Brand Name] SSMT Export.xlsx` and generates the uploaded Centric export shape with `Glossary`, `Brand`, `Menus`, `Categories`, `Items`, `Modifier Groups`, `Modifiers`, and `Relationships` tabs. Exported items, categories, area prices, modifiers, relationships, and exact MRN text come from the app-side selected SSMT menu and modifier records; Centric-owned ID columns are blank and unknown optional business fields use `[placeholder]` for IT review. SSMT also now loads and saves normal workspace edits through the shared `ssmt|workspace|current` app_records path before falling back to guarded browser cache. The seed parser now recovers screenshot-aligned Global menus from consolidated workbook tabs including `Globals`, `Microconcept`, `Street Eat Concepts`, and `Pho and Yakisoba (Noodles)`, maps worksheet names to cleaner display names, and preserves the selector's alphabetical sort inside each group. The regenerated seed contains 36 Global menus, 34 Core menus, 11 Promotion menus, and 5 Thompson Hospitality menus. Local focused guards passed: `node scripts/test-storage-backbone.mjs`, `node scripts/verify-ssmt-centric-export.mjs`, `node scripts/build-ssmt-seed-data.mjs --check`, `node scripts/verify-menu-audit-tool.mjs`, `node scripts/verify-platform-tools.mjs`, `node scripts/verify-menu-projects.mjs`, `node scripts/verify-release-workflow.mjs`, `node scripts/verify-changelog-timestamps.mjs`, `git diff --check`, `node scripts/run-playwright.mjs tests/browser/ssmt-tool.spec.js` 10/10, and `corepack pnpm run build`. Production Playwright passed `node scripts/run-playwright.mjs tests/browser/ssmt-tool.spec.js` 10/10 against `https://project-d8v25.vercel.app`. Full `corepack pnpm run verify` remains blocked before SSMT by the pre-existing stale generated food-cost reference guard. No Supabase/schema/production-data, Webtrition/MenuWorks authority, pricing-source, area price order, MRN coercion, deletion authority, or downstream eligibility change. Release state: LIVE through a Tyler-authorized direct-Chief release path.

## 2026-08-30

- [Aug 30, 9:37 PM] Prepared Tyler's SSMT shared workspace persistence follow-up as version `2026.08.30.003-ssmt-shared-workspace`. This candidate was published to production as part of the August 31, 2026 `2026.08.31.001-ssmt-centric-export` release above. SSMT now loads `/api/storage/records?tool=SSMT` before falling back to guarded browser cache or seed data, and saves the normal SSMT workspace back to one shared `ssmt|workspace|current` app_records record so menus, item locks, phase/status, pricing rows, modifier groups, and selected-menu state can be shared across users/devices. The storage API treats `SSMT` as a logical tool scope while routing the physical row through the existing compatible app_records bucket, so this candidate does not require a production schema migration. A small shared-workspace status line now tells users whether shared storage loaded/saved or local fallback is being used. Local TDD proof first failed because SSMT was not recognized as a storage scope and the browser could not load `Shared Lock Menu` from shared storage; after implementation, `node scripts/test-storage-backbone.mjs` passed and `node scripts/run-playwright.mjs tests/browser/ssmt-tool.spec.js` passed 9/9, including shared locked-row load and unlock-save proof. No manual production-data rewrite, Webtrition/MenuWorks authority change, pricing-source change, area price order change, MRN coercion, modifier authority change, deletion authority change, or downstream eligibility change. Release state: LIVE as part of the August 31 release above.

- [Aug 30, 7:02 PM] Published Tyler's SSMT width-lock, item lock, copy-to-Centric, phase prominence, price readability, and row-striping follow-up to GitHub `main` at application commit `87ea9e8073f712c1460f83f379b060c06b2f8501` as version `2026.08.30.002-ssmt-lock-copy-phase`. Vercel production deployment `dpl_Boy5EkSwa57DNV5Qqn7xBTgq3rGb` is `READY`; live HTML references `/assets/index-CUaQZ8c1.js`, that live index contains the version stamp, and the live SSMT chunk `/assets/SsmtTool-LIJ4engG.js` contains the lock/phase/width markers. The unlocked SSMT workspace keeps the dense, wide menu editor but caps the desktop surface at a readable maximum so a maximized ultra-wide browser no longer stretches the builder across the full viewport. The phase/status control is now a prominent panel under the editor navigation, item rows can be locked when Culinary finishes entering them, locked fields become read-only click-to-copy fields for Centric, area-price cells are larger and copyable when locked, locked rows cannot be edited/repriced/deleted until unlocked, and forward phase movement is disabled until every item row is locked. Builder rows also use a clearer alternating row color. Production Playwright passed `tests/browser/ssmt-tool.spec.js` 8/8 against `https://project-d8v25.vercel.app`; direct local SSMT browser proof first failed against missing lock/copy behavior and the previous over-stretched 2966.5px builder width at a 3000px viewport, then passed after implementation. No Supabase/schema/production-data, shared SSMT persistence authority, pricing-source authority, workbook parsing, Menu Audit source framing, area price order, exact MRN handling, modifier behavior, or Webtrition/Menu Library deletion authority changed. Full `corepack pnpm run verify` remains blocked by the pre-existing stale generated food-cost reference guard. Release state: LIVE.

## 2026-08-28

- [Aug 28, 3:20 PM] Published Tyler's SSMT maximized-screen density follow-up to GitHub `main` at application commit `a2f4adb1db522c90e7a4398dffe51e788be56e42`. Vercel production deployment `dpl_Z94rhLWbhnQ42iVZgQp5iN7eQ7CP` completed successfully and is aliased to `project-d8v25.vercel.app`; live HTML references `/assets/index-CiXuwr92.js`, and that live asset contains version `2026.08.28.004-ssmt-maximized-builder-density`. SSMT now uses the full desktop work surface instead of capping the editor in a centered column, trims top-panel spacing, compacts main builder row fields/actions, uses tighter eight-column area-price grids, and darkens builder row borders so a maximized browser shows more of the menu at once. This includes the modifier popup density pass from the immediately preceding `.003` candidate. Production Playwright passed `tests/browser/ssmt-tool.spec.js` 7/7 against `https://project-d8v25.vercel.app`. No Supabase/schema/production-data, modifier persistence authority, pricing-source authority, workbook parsing, Menu Audit source framing, area price order, exact MRN handling, or Webtrition/Menu Library deletion authority changed. Full `corepack pnpm run verify` remains blocked by the pre-existing stale generated food-cost reference guard. Release state: LIVE.

- [Aug 28, 3:02 PM] Prepared Tyler's SSMT modifier item-line density follow-up as version `2026.08.28.003-ssmt-modifier-row-density`. The modifier popup keeps the wider desktop dialog and green `View modifiers` action, while modifier item rows now use tighter cell padding, smaller controls, a shorter resizable description field, an eight-column area-price grid, compact delete action, and darker item/group borders so more modifier lines fit on screen. No Supabase/schema/production-data, modifier persistence authority, pricing-source authority, workbook parsing, Menu Audit source framing, area price order, exact MRN handling, or Webtrition/Menu Library deletion authority changed. Local TDD proof first failed against the previous 185px modifier item row height, then passed after implementation; local SSMT Playwright passed `node scripts/run-playwright.mjs tests/browser/ssmt-tool.spec.js` 6/6.

- [Aug 28, 7:57 AM] Published Tyler's SSMT modifier popup usability follow-up to GitHub `main` at application commit `4e27a3e7782ec08f6d8b23f0baf5a0afea51b380`. Vercel production deployment `dpl_5gE6UyxescoeekR5qGAFCXQzczeM` completed successfully and is aliased to `project-d8v25.vercel.app`; the live public bundle contains version `2026.08.28.002-ssmt-modifier-dialog-width`, live HTML references `/assets/index-ZzzpQGtq.js`, and the live SSMT chunk `/assets/SsmtTool-BwIsP9jc.js` contains the widened modal and green modifier action classes. Production Playwright passed `tests/browser/ssmt-tool.spec.js` 6/6 against `https://project-d8v25.vercel.app`, including the wider modifier dialog and green `View modifiers` button. No Supabase/schema/production-data, modifier persistence authority, pricing-source authority, workbook parsing, Menu Audit source framing, or Webtrition/Menu Library deletion authority changed. Full `pnpm run verify` remains blocked by pre-existing stale generated-data checks. Release state: LIVE.

- [Aug 28, 7:52 AM] Prepared Tyler's SSMT modifier popup usability follow-up as version `2026.08.28.002-ssmt-modifier-dialog-width`. The row-level `View modifiers` action now uses a green high-contrast treatment so it stands out from secondary row actions, and the modifier detail popup now opens in a wider desktop dialog with a tightened fixed modifier table layout so normal desktop use does not require sideways scrolling around the modifier name, description, MRN, calories, price, area prices, and delete action columns. Existing modifier group creation, rename/type, copy, delete, item-line editing, exact MRN text, pricing-structure selection, area price order, SSMT app-side persistence, Supabase/schema state, production data, Menu Audit source framing, and Webtrition/Menu Library deletion authority are unchanged. Local TDD proof first failed on the existing white modifier button, then passed `node scripts/run-playwright.mjs tests/browser/ssmt-tool.spec.js` 6/6 after implementation.

- [Aug 28, 7:13 AM] Published Tyler's requested Platform Home sectioning and `SSMT` usability/modifier follow-up to GitHub `main` at application commit `89ecbc1d03b00e9d270d9d4bf448e9845e29113b`. Vercel production deployment `dpl_GtDEJHDd5D7BmjzzQjbQmXcYgVhf` completed successfully and is aliased to `project-d8v25.vercel.app`; the live public bundle contains version `2026.08.28.001-ssmt-home-sections-modifiers`. Production Playwright passed the combined focused suites 6/6 against `https://project-d8v25.vercel.app`, covering the requested home section headers and exact tool order, SSMT passcode flow, pricing/menu split, dense four-column menu selector, visible internal wide-table scrolling, wider item text fields, simple click-confirm deletion for app-side SSMT menu/item/modifier records, modifier group create/rename/type/copy/delete, modifier item add/delete, exact MRN text, pricing-structure selection, area price order through MCO, and unchanged Menu Audit source framing. No Supabase/schema/production-data, Webtrition/Menu Library deletion authority, pricing-source authority, workbook parsing, Menu Audit removal authority, Menu Cross scoring/data logic, or SSMT passcode model changed. Full `pnpm run verify` remains blocked by pre-existing stale generated-data checks. Release state: LIVE.

- [Aug 28, 7:07 AM] Prepared Tyler's requested Platform Home sectioning and `SSMT` usability/modifier follow-up as version `2026.08.28.001-ssmt-home-sections-modifiers`. Platform Home now groups tools under `Chef Tools` in the order `Neighborhood Rotations`, `Menu Library`, `Menu Engineering`, `Menu Cross Utilization Tool`, `Webtrition`, then `Programming & Auditing` in the order `SSMT`, `Menu Projects`, `Menu Audit Tool`, `Lean Tool`. SSMT now uses a denser four-column menu selector with stronger menu-type contrast, keeps the wide builder table in an internal scroll region, widens/wraps editable item text fields, replaces exact-name deletion with simple click-confirm deletion for app-side SSMT menu/item/modifier records, and lets users create, rename, type, copy, and delete modifier groups plus add/delete modifier item lines with modifier name, description, exact MRN text, calories, pricing-structure selection, and area prices in the required order `AUS, BNA, BOS, BWI, DEN, IAD, JFK, LAX, SAN, SNA, SEA, SJC, WAS, YVR, YYZ, MCO`. No Supabase/schema/production-data, Webtrition/Menu Library deletion authority, pricing-source authority, workbook parsing, Menu Audit removal authority, Menu Cross scoring/data logic, or SSMT passcode model changed. Local browser proof passed `node scripts/run-playwright.mjs tests/browser/landing-tool-sections.spec.js` 1/1 and `node scripts/run-playwright.mjs tests/browser/ssmt-tool.spec.js` 5/5 after implementation.

## 2026-08-27

- [Aug 27, 8:11 PM] Published Tyler's requested `SSMT` menu-selector and row-control refinements to GitHub `main` at application commit `9025b09affbe38792706b882158f7ac6c95c07aa`. Vercel production deployment `dpl_Cfi71ZvBUjDvoy7vqwaL6EfJf5Lk` completed successfully and is aliased to `project-d8v25.vercel.app`; the public bundle `/assets/index-Bu84N9fV.js` contains version `2026.08.27.002-ssmt-menu-row-ordering`. Production Playwright passed `tests/browser/ssmt-tool.spec.js` 3/3 against `https://project-d8v25.vercel.app`, covering passcode, split SSMT navigation, grouped/color-coded menu selector, alphabetical menu ordering, new menu creation, phase/status persistence across reload, add item, add divider, drag/drop ordering, editable Fixy/MRN/category/secondary category fields, non-Promotion calories as `N/A`, item delete, modifier modal/copy, flag report, exact-name menu delete, and unchanged Menu Audit source framing. No Supabase/schema/production-data, pricing-source authority, workbook parsing, Menu Audit removal-authority, or downstream Core/Global eligibility change occurred. Full `pnpm run verify` remains blocked by pre-existing stale generated-data checks. Release state: LIVE.

- [Aug 27, 7:56 PM] Prepared Tyler's requested `SSMT` menu-selector and row-control refinements as version `2026.08.27.002-ssmt-menu-row-ordering`. Menu Selector now groups records in the approved order `Core`, `Global`, `Promotions`, `Thompson Hospitality`, color-codes each group, and alphabetizes menus inside each group. The menu builder now supports adding menu items, deleting item rows, and dragging/dropping item or divider rows to set the IT programming order. The item table now puts blank editable `Fixy` left of `Label`, makes `MRN` a blank editable text field, removes the visible `FOH` label, keeps calories as `N/A` for non-Promotion menus, and places blank editable `Category` and `Secondary category` after SEA price before area prices. Phase/status changes are saved on each app-side menu record and now survive SSMT reload through guarded browser persistence. No Supabase/schema/production-data, pricing-source authority, workbook parsing, Menu Audit removal-authority, or downstream Core/Global eligibility change occurred. Local SSMT browser proof first failed on missing grouped menu sections, then passed `node scripts/run-playwright.mjs tests/browser/ssmt-tool.spec.js` 3/3.

- [Aug 27, 12:40 PM] Published Tyler's requested `SSMT` flow restructure to GitHub `main` at application commit `68db163d6fedcd51e35b45e668b2e0ed86945cc4`. Vercel production deployment `dpl_3vhmQemNEVmcHhBjxuL86JxbhvLX` completed successfully and is aliased to `project-d8v25.vercel.app`; the public bundle `/assets/index-CvnbshjJ.js` contains version `2026.08.27.001-ssmt-flow-restructure` plus the new `Pricing Structure` and `Menu Selector / New Menu` labels. Production Playwright passed `tests/browser/ssmt-tool.spec.js` 2/2 against `https://project-d8v25.vercel.app`, covering passcode, split navigation, pricing row add, menu create/type selection, full-width menu editor fields, dividers, exact-name delete, Promotion/Thompson-only active dates, modifier copy, flag report, and unchanged Menu Audit source framing. No Supabase/schema/production-data, seed parsing, pricing-source authority, Menu Audit removal-authority, or workbook deletion-authority change occurred. Full `pnpm run verify` remains blocked by the pre-existing stale generated food-cost reference guard before it reaches SSMT. Release state: LIVE.

- [Aug 27, 12:27 PM] Prepared Tyler's requested `SSMT` flow restructure as version `2026.08.27.001-ssmt-flow-restructure`. After passcode `0411`, SSMT now opens to `Pricing Structure` and `Menu Selector / New Menu` paths so pricing maintenance is separated from menu programming. The pricing view shows the required IT area order `AUS, BNA, BOS, BWI, DEN, IAD, JFK, LAX, SAN, SNA, SEA, SJC, WAS, YVR, YYZ, MCO` and can add app-side SEA price + category rows. The menu selector can open workbook-derived menus or create a new menu after selecting type. The menu editor is full-width, adds a slim selected-menu nav for back to selector, pricing table, and delete menu, requires exact menu-name retyping before deletion, adds typed Category and FOH / Fixy fields, displays all area prices per item in IT order, and supports titled divider rows for Centric categories inside one menu. Active start/end dates now appear only for Promotion and Thompson Hospitality menus; Core and Global show that active dates are not required. Menu Audit source framing, workbook parsing, pricing-source authority, Supabase/schema state, production data, and deletion authority are unchanged. Local SSMT browser proof first failed on the missing post-passcode two-button split, then passed `node scripts/run-playwright.mjs tests/browser/ssmt-tool.spec.js` 2/2.

## 2026-08-26

- [Aug 27, 7:48 AM] Published the new `SSMT` tool and Menu Audit source-model reset approved by Tyler. Platform Home now includes a passcode-gated `SSMT` tool (`0411`) that loads the current workbook-derived seed data as a static JSON payload, shows the required area order `AUS, BNA, BOS, BWI, DEN, IAD, JFK, LAX, SAN, SNA, SEA, SJC, WAS, YVR, YYZ, MCO`, lets users assign item prices through pricing-structure SEA price + category selectors, keeps unmatched workbook item prices as review context instead of assigned prices, keeps active start/end dates editable on the menu record for calendar/status tracking, normalizes item labels to ALL CAPS, normalizes descriptions to lower case, keeps calories promotional-only, opens modifier detail in a modal, records copied modifiers as independent-group behavior, and creates saved flag/report email drafts to `alexander.neuse@compass-usa.com` and `tyler.leiss@compass-usa.com`. Added `scripts/build-ssmt-seed-data.mjs` to parse `docs/ssmt/SEA Standard Menu Template (1).xlsx` into `public/data/ssmtSeedData.json` and guard freshness in `pnpm run verify`; the parser reads Navigation UI drawing hyperlink targets plus approved visible recent/promotion sheets, and the parsed seed currently contains 56 menu sheets, 592 modifier groups, 62 pricing rows, and 61 Navigation UI button targets. Menu Audit now presents the future operating source model as SSMT app records + Webtrition Report Menu Index + Shopping Lists, calls out recipes as the remaining missing data layer, and keeps SSMT workbook-only differences as review flags rather than deletion authority. Application commit `79a0656b0e1f38fdc691f6731266e6860d11a125` reached GitHub `main`; Vercel production deployment `dpl_82oFq2v5Ga5AbTUn3qvXkwGZ3K9Z` completed successfully, the public bundle contains version `2026.08.26.001-ssmt-tool-menu-audit-source`, live seed data matches the 56/592/62/61 counts with zero unmatched raw workbook prices assigned, and production Playwright passed `tests/browser/ssmt-tool.spec.js` 2/2. Normal Reviewer, Verifier, and Release sessions were unavailable with expired OAuth; Tyler explicitly instructed Chief to continue directly. Release state: LIVE.
