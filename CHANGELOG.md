...
- [Aug 28, 3:20 PM] Prepared Tyler's SSMT maximized-screen density follow-up as version `2026.08.28.004-ssmt-maximized-builder-density`. SSMT now uses the full desktop work surface instead of capping the editor in a centered column, trims top-panel spacing, compacts main builder row fields/actions, uses tighter eight-column area-price grids, and darkens builder row borders so a maximized browser shows more of the menu at once. This includes the modifier popup density pass from the immediately preceding `.003` candidate. No Supabase/schema/production-data, modifier persistence authority, pricing-source authority, workbook parsing, Menu Audit source framing, area price order, exact MRN handling, or Webtrition/Menu Library deletion authority changed. Local TDD proof first failed against the previous 1943px maximized builder width and 185px modifier item row height, then passed after implementation; local SSMT Playwright passed 7/7 and focused source/release/build guards passed. Full `corepack pnpm run verify` remains blocked by the pre-existing stale generated food-cost reference guard.
