...
- **SSMT verification must never write to the live workspace record (permanent, since 2026-09-19):** on 2026-09-19, a browser regression test for the shared-Sub-Menu-membership feature navigated to the app and saved before the real shared `ssmt|workspace|current` load had finished, overwriting production with a mid-load seed payload. That wiped every menu's bucket/phase assignment and all Divider/Sub Menu structural rows; menu/item/modifier/pricing content was not lost. Recovery required a paid Supabase point-in-time restore, which Tyler declined; the workflow-state loss was not recoverable for free. Going forward: any browser test, script, or "production verification" step that exercises SSMT save/load must intercept `**/api/storage/records**` (see the mocked-route pattern already used throughout `tests/browser/ssmt-tool.spec.js`, e.g. the "shares one item across multiple sub menus" and "loads and saves item lock state through shared storage" tests) or otherwise target an isolated non-production record. Never `page.goto` into the live app and exercise a real Save against `ssmt|workspace|current` as a verification step. Any intentional production SSMT data write requires Tyler's explicit approval immediately beforehand plus a verified recovery path. (Separately, several older tests in that same file — flagged pre-existing, not fixed by this rule — still write throwaway menus to the real live app when Playwright's `PLAYWRIGHT_BASE_URL` targets production; see the smoke-test menu-leak note keyed to the same file.)
